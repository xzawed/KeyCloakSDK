...
> **`1.1.1` is on crates.io** — a patch release on top of `1.1.0`; `1.0.0` was the first release that carries the stability guarantee (a breaking change to the public API requires a major bump). The bare `cargo add keycloak-sdk` below resolves `1.1.1`, and a hand-written `keycloak-sdk = "1"` or `keycloak-sdk = "1.0"` matches it too. **Fixed**: when the token or introspection endpoint echoes a token back in its error response, the SDK error no longer prints it — `{:?}` showed the `error_description` and `error_uri`, and `{}` quoted the response's Content-Type value, an id_token's `kid` and values from a JWKS body; those messages now keep what failed and where, without the value. ⚠️ **`KeycloakError::Auth`'s `oauth_error` now carries only a code-shaped OAuth error code** (such as `invalid_grant`), or `None` — from `AuthClient` (code exchange, refresh, client credentials, introspection) it used to hold the whole `error: error_description (see error_uri)` text whenever the server sent a description, so a comparison against an exact code that failed before now matches. No public API changed.
...
- **DoS-safe JWKS** — keys are cached, a refetch is triggered only by an unresolved `kid` and never by a bad signature, and it is rate-limited to a minimum interval (`with_jwks_min_refetch_secs`, 30s by default). The gate is stamped when the refetch is *decided*, not when it succeeds, so an IdP outage cannot reopen it — so **once the key set has been fetched**, no volume of forged `kid`s makes the SDK issue more than one JWKS request per interval. A *failed* fetch is bounded separately: consecutive failures back off exponentially (0.2s, doubling, capped at 5s, with jitter), and inside that window the SDK **fails fast without contacting the IdP**. So a cold cache during an IdP outage no longer turns every validation into a request (measured: 20 attempts → 1 request, down from 20). Concurrent first validations collapse to a single fetch as well (measured: 20 concurrent → 1). ⚠️ The SDK never sleeps — it returns the error immediately, so retry pacing stays the caller's decision.
