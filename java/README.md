...
> **`1.0.2` is on Maven Central** — a patch release of security and correctness fixes: a `200` JWKS response carrying an empty key set no longer replaces a good cached one, a public (secret-less) client can now `refresh` and `logout`, and bad call arguments or config values surface as `KeycloakAuthException` / `KeycloakConfigException` instead of leaking Nimbus exceptions. ⚠️ **Config is now validated when it is built** — `serverUrl` must be an absolute http(s) URL, and a connect or read timeout of `0` (an unbounded wait until now) is rejected. No public API changed. ⚠️ **Maven never picks a version for you**: a `<dependency>` with no `<version>` and no BOM managing it fails the build rather than resolving to the newest release, so name `1.0.2` explicitly as shown below.

## Requirements

- **JDK 17+** — artifacts are compiled with `--release 17`, so an older JDK raises `UnsupportedClassVersionError`. ⚠️ **`1.0.0` needed JDK 21**; `1.0.1` is the release that lowered the floor.
- Maven 3.9+.
...
Masking covers this SDK's own `toString()`. ⚠️ It does **not** cover serialization: these types carry no serialization hook, so Jackson or Gson reading their getters emits the raw value — [SECURITY.md](../SECURITY.md) lists exactly that as a measured gap, and this line used to claim the opposite. Nor can masking cover what your logging framework, a debugger, or a heap dump does with a value you hand it. The `char[]` secret is defence in depth rather than an erasure guarantee — `keycloak-admin-client` and Nimbus both take `String`, so the secret is copied into an unerasable heap string at the point of use.
