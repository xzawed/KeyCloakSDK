...
- ⚠️ **Four kinds of pin dependabot must not bump** — the `ignore` list in `.github/dependabot.yml` blocks them, with the reasoning attached. ⚠️ **A bump its constraint forbids makes dependabot *error*, not skip**, so leaving one out of `ignore` reddens that ecosystem's whole job every week — and the failure shows up only under Insights → Dependency graph → Dependabot, never in CI. ⚠️ **Not a partition** — `@types/node`·`vitest`·`@vitest/coverage-v8` are held for reasons stated beside them.
  1. **An action whose ref name *is* the meaning.** `dtolnay/rust-toolchain` is pinned to the head SHA of the `stable` **branch**; dependabot swaps in the default-branch head and the workflow dies with `'toolchain' is a required input`. `pypa/gh-action-pypi-publish` is worse — its default branch is `unstable/v1`, so instead of dying, **PyPI publishing quietly moves to the unstable channel**. When bumping, read the branch head: `gh api repos/<owner>/<repo>/branches/<branch> --jq .commit.sha`.
  ...
  3. **A version that expresses a CI-matrix floor.** `parallel` 2.x needs ruby `>= 3.3` while `ruby-ci` still runs a **3.2** leg, so `Gemfile.lock` stays on 1.x — hence `gem "parallel", "< 2"`. The value is low because of the *test matrix*, not the code, so nothing in `ruby/` explains it; the reason sits in `dependabot.yml`. Drop both when the gemspec floor passes 3.2.
  4. **A version that tracks a target server line, not semver.** cargo `keycloak` follows the Keycloak **server** line, so a minor bump moves the target server and integration/harness must move with it. Patches only.
