...
- Also watch for: **three** pairs of colliding context names (enumerated in CONTRIBUTING §4 — do not copy it here; it drifted twice because the number is a *measurement*, not a constant) and the absence of a `merge_group:` trigger (turn the merge queue on and everything deadlocks). Resolutions: [CONTRIBUTING.md §4](../../CONTRIBUTING.md).
...
- ⚠️ **Measure a guard's detection surface with `scripts/probe.sh`, not by hand.** It is fail-closed and splits three outcomes that hand-rolled probes conflate: `0` caught · `1` a real hole · `2` **the probe was invalid** (dirty tree, already-red baseline, or a mutation that never landed). It refuses to start on uncommitted work and mutates only a throwaway worktree. Measured cost of not having it: a blanket `git checkout -- .` destroyed finished uncommitted work twice in one session, and two "the guard is silent" findings were false — the mutation had never applied. ⚠️ **Do not reimplement an oracle to measure it — run it.** A hand-written re-implementation of the doc-budget metric was off by ~9,900 bytes; `check-docs.mjs` itself gave the real number.
