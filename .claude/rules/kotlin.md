...
- 커버리지 게이트(Kover, 네트워크 경계 omit): `gradle -p kotlin koverVerify` — **실측 라인 99.24%/브랜치 85.71%**(omit 대상 `AuthClient*`/`admin.*`/`KeycloakClient*` — 통합 E2E로 검증).
...
- ⚠️ **(Kotlin) 로컬 포터블 Gradle과 CI 래퍼 버전을 일치시켜 둔다(현재 둘다 9.6.1)** — 어긋나면 로컬에서 재현 안 되는 CI실패 발생.
- ⚠️ **(Kotlin) KGP 2.4.10의 Gradle 완전지원 밴드는 `7.6.3`–`9.5.0`이고, 우리 래퍼 `9.6.1`은 이미 그 위다.** 실측(kotlinlang.org "Apply the plugin" 호환표 원문): `2.4.0-2.4.10 | 7.6.3–9.5.0 | AGP 8.5.2–9.1.0`, 본문 "the maximum fully supported version is 9.5.0". 같은 문서가 "You can also use Gradle … up to the latest releases, but … you might encounter deprecation warnings or some new features might not work"라 **밴드 밖이 곧 고장은 아니다**(현재 CI 초록). ⚠️ **밴드 밖으로 나간 것은 결정이 아니라 사고였다** — `723d0a4`(dependabot `kotlin-minor-patch` 그룹)가 KGP `2.2.20→2.4.10`과 래퍼 `9.5.0→9.6.1`을 **한 커밋에** 올렸고, 이 줄이 요구하던 밴드 확인은 그때 아무도 하지 않았다. 그래서 dependabot #183(`9.6.1→9.7.0`)은 **기각**했다: 9.7.0은 밴드를 좁히는 게 아니라 한 칸 더 벌린다(Gradle 9.7.0이 임베드하는 Kotlin은 2.4.0이고 이는 빌드스크립트 DSL 축이라 KGP 지원과 별개다). **되살릴 조건**: KGP가 2.4.20+ 등으로 올라 지원 상한이 9.7.0 이상이 되었을 때 — 그때 KGP와 래퍼를 **함께** 올린다. 그 전에 래퍼만 올리지 않는다. ⚠️ 하네스 소비자앱(`harness/apps/kotlin`·`harness/install/consume/kotlin-app`)은 아직 `9.5.0`이라 리포 안에 Gradle이 두 버전 있다 — 하네스는 소비자 입장을 흉내내므로 의도적으로 낮은 쪽이 맞지만, 위 밴드를 되찾는 방향으로 `kotlin/`을 9.5.0으로 되돌리는 선택지도 열려 있다(사람 판단 — 다른 PC의 포터블 Gradle 교체가 따라와야 한다).
