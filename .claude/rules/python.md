...
- ⚠️ **JWKS does not go through upstream `certs()` / `a_certs()` — it is a byte-capped direct GET** (`_internal/jwks_fetch.py`, 51200 = the nine-language cap). Upstream offers no seam for a cap: `raw_get` forwards `**kwargs` as `params=`, so `stream=True` becomes a query parameter. Capping the whole session is wrong — token/refresh/exchange_code/logout/introspect share it and a role-heavy access token is legitimately large. **Two consequences to keep in mind when editing this path.** (1) ⚠️ **`_wrap` / `_awrap` translate only upstream `keycloak.exceptions.KeycloakError`**, so a direct HTTP call must translate `requests`/`httpx`/`json` exceptions itself or a foreign type escapes (§4). (2) ⚠️ **the two mirrors use different clients** — sync `connection._s` (requests, redirect-hardened by `harden_openid`), async `connection.async_s` (httpx, where the defence is httpx's `follow_redirects=False`, which `afetch_jwks` therefore states explicitly). A JWKS 3xx/non-2xx surfaces as `KeycloakTransportError`, not `KeycloakAuthError` — the other lanes still give `KeycloakAuthError` because they go through upstream's `raise_error_from_response`.
