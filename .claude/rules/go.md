...
- ⚠️ **(Go) dev-dependency 개념이 없다 — `// indirect`를 "우리가 고른 의존성"으로 읽지 말 것.** 루트 `CLAUDE.md`가 한때 `github.com/stretchr/testify` v1.11.1을 우리가 선택한 test 의존성처럼 나열했으나 **go 코드는 testify를 import하지 않는다**(`grep -rn stretchr/testify go/*.go` → 0건) — `testcontainers-go`가 끌어온 전이 의존성이고 `go.mod`에도 `// indirect`로 적혀 있다. 의존성 표에는 우리가 실제로 import하는 모듈만 담는다. `go-oidc`는 애초에 제외했다 — discovery는 규약 조립이고 verifier는 go-jose로 자체 강화하므로 얻을 것이 없다.
