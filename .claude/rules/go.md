...
- ⚠️ **The minimum Go is 1.26** (required by `golang.org/x/oauth2` v0.37 — lower `go.mod` and `go mod tidy` puts it back). CI runs this minimum as a leg. ⚠️ **`golangci-lint` runs nowhere in CI** — `.golangci.yml` is a local-only config. The CI gates are `gofmt` · `go vet` · `staticcheck` · `gosec`, plus `govulncheck` in its own job, so vet and gofmt alone do **not** stand in for them.
