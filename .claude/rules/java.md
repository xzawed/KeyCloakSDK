...
- JDK 21.0.11 (Eclipse Temurin) · Maven 3.9.9 (머신 전용 경로 — 리포지토리에 커밋 안 함, CI는 setup-java 사용). ⚠️ 이 줄은 위 프리픽스(`jdk-21.0.11.10-hotspot`)와 **같은 사실의 두 번째 자리**라 갈린 채 굳었던 적이 있다(여기만 `21.0.8`로 남아 있었다). 고칠 때 둘 다 본다 — 실측은 `java -version`.

## 게차

- ⚠️ **(Java) 퍼블릭/PKCE 클라이언트에서 `new String((char[]) null)`은 맨 NPE다.** `KeycloakConfig.getClientSecret()`은 `char[]`이고 퍼블릭 클라이언트에서는 null인데, 이를 무조건 문자열화하던 `AuthClient.clientAuth()`가 client-credentials·refresh·logout·introspect 네 경로 전부에서 진단 불가능한 NPE를 던졌다. 지금은 호출 흐름 이름을 인자로 받아 "이 작업은 기밀 클라이언트를 요구한다"는 `KeycloakConfigException`을 던진다(IdP 거절이 아니라 로컬 구성 미비 — `AuthClientPublicClientTest`가 타입을 고정). **`char[]` 시크릿을 문자열화하는 지점은 전부 null 가드가 선행해야 한다** — 같은 부류의 버그를 다른 경로/언어에 재도입하지 말 것.
...
- ⚠️ **(Java) `admin`은 토큰을 자체 소유한다** — `AdminClient(KeycloakConfig)`가 admin-client 내장 client-credentials를 쓰고, `TokenProvider` 기반 생성자는 RESTEasy 필터 충돌로 제거됐다(`AdminClient` javadoc). `ClientCredentialsTokenProvider` 타입은 auth 모듈에 남아 있으나 파사드가 배선하지 않는다. admin은 auth를 직접 알지 못한다(§4).

- ⚠️ **admin-client와 Keycloak 서버는 독립 버전 트랙이다 — 서버 라인과 같은 번호의 admin-client는 존재하지 않는다**(핀은 루트 `CLAUDE.md`「확정 의존성」이 SSOT). `representation` 필드가 서버와 불일치할 수 있어, 의존하는 필드는 실서버로 검증한다. Kotlin도 같은 좌표를 재사용하므로 같은 제약을 받는다.
- ⚠️ **admin 타임아웃·자원정리.** `AdminClient`는 connect/read 타임아웃을 `KeycloakBuilder.resteasyClient(...)`로 주입해야 무한대기를 막는다(미주입 = 스레드 고갈 DoS). `close()`/`aclose()`는 admin뿐 아니라 **auth 세션(requests/httpx)까지** 정리한다(미정리 = FD/커넥션풀 누수). ⚠️ 단 `resteasyClient(...)` 주입은 admin-client의 `JacksonProvider` 등록을 통째로 우회하므로 `buildTimeoutClient`가 프로바이더를 직접 등록한다(위 게차 참고).
- ⚠️ **jackson-databind는 2.22.1 고정**(`dependencyManagement`, dependabot 유지). CVE 대응 이력: 2.21.2 → 2.21.4 → 2.21.5[CVE-2026-54515] → 2.22.1. **보안 불변식**: 자체 `ObjectMapper`나 default/polymorphic typing을 쓰지 않고, 신뢰된 Keycloak 응답만 고정 POJO로 역직렬화한다 — default typing 활성화 · 커스텀 JAX-RS Jackson provider 등록 · 미신뢰 JSON 다형 역직렬화 도입 **전부 금지**다.
