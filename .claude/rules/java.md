...
- ⚠️ **Stringifying a `char[]` secret unconditionally is a bare NPE on a public/PKCE client.** `getClientSecret()` is null on a public client, and `AuthClient.clientAuth()` passed it straight into `new String(...)`, producing an undiagnosable NPE. Now only client-credentials and introspect throw a `KeycloakConfigException` (Keycloak refuses public clients there too: 401/403); **refresh and logout send `client_id` in the body instead — Keycloak allows both for public clients** (measured on 26.6; `AuthFlowIT` pins it against a real server, Kotlin `FullFlowIT` likewise). **Every point that stringifies a `char[]` secret must be preceded by a null guard.**
