...
- ⚠️ **JaCoCo checks each module separately, so the repository total hides the module that is actually at risk.** `keycloak-sdk-auth` sat at **exactly 85.00% branches** (17 of 20, gate 85) while the four modules summed to a comfortable 93.9%. Read the per-module figure, not the sum. It is now 42/44 with 4 branches of slack.
...
- ⚠️ **Stringifying a `char[]` secret unconditionally is a bare NPE on a public/PKCE client.** `getClientSecret()` is null on a public client, and `AuthClient.clientAuth()` passed it straight into `new String(...)`, an undiagnosable NPE. Only client-credentials and introspect throw `KeycloakConfigException` (Keycloak refuses those too); **refresh and logout send `client_id` — Keycloak allows them** (pinned by `AuthFlowIT`). **Every point that stringifies a `char[]` secret must be preceded by a null guard.**
