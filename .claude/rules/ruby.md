...
- ⚠️ **(Ruby) `create_authorization_request`의 `nonce:` 기본값은 `state:`와 같다** — `SecureRandom.urlsafe_base64(24)`로 **항상** 생성해 URL에 싣는다. 예전에는 `nonce: nil`이 기본이라 호출자가 넘겨야만 URL에 실렸다(아홉 중 유일). `exchange_code(expected_nonce:)`는 그대로 옵셔널(여덟 공통 — 생략 시 id_token 검증 스킵). `AuthorizationRequest#inspect`는 nonce를 state처럼 노출하고 `code_verifier`만 마스킹한다(인가 URL에 이미 실리는 재생 방지 값).
