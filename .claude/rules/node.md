...
- ⚠️ **`tsconfig.json` sets `include: ["src"]`, so the test files are never type-checked** — they can keep importing a type that no longer exists and typecheck, eslint and vitest all miss it (esbuild strips the types). That is a blind spot when verifying a dependency major bump. Adding `test` to `include` surfaces pre-existing errors, so it needs its own piece of work.
