...
- Coverage (logic lines ≥90%, network boundary omitted): `rustup component add llvm-tools-preview`, then `cargo llvm-cov --ignore-filename-regex '(auth|admin|client)\.rs' --fail-under-lines 90`. Measured 96.48% (35 of 993 lines unexecuted).
  ...
  - ⚠️ **There is no branch-coverage gate here, and there cannot be on the pinned toolchain.** `cargo llvm-cov --branch` exists but its help marks it *(unstable)*: it passes `-Z coverage-options=branch` to rustc, and on stable that dies with `error: the option \`Z\` is only accepted on the nightly compiler` (measured on 1.97.1). The `Branches` column of a normal `--summary-only` run therefore reads `0 0 -` — that is "not collected", not "none exist". Six of the nine SDKs gate branches, so this gap looks like an omission and invites a "fix"; it is a toolchain constraint. **Revival condition**: `-Z coverage-options=branch` **lands on stable** (not merely loses the *(unstable)* label in help). ⚠️ The reason not to reach for nightly is **policy, not MSRV** — the coverage job already carries its own `dtolnay/rust-toolchain` pin separate from the `1.88 · stable` test matrix, so a nightly coverage job would not touch `rust-version`. It is refused because `-Z` flags are a moving target, it adds a second toolchain pin for dependabot to track, and the resulting percentage would not be comparable to the JaCoCo/Kover 85 the other six gate against.
