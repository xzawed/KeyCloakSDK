...
- ⚠️ **(Rust) admin 경계 변환 함수는 `map_admin`이다** — `keycloak::KeycloakError`가 여기서 SDK `KeycloakError`로 바뀐다. `TokenProvider` trait(async)를 `AuthClient`가 구현하고 `SdkTokenSupplier`가 그것을 crate의 `KeycloakTokenSupplier`로 어댑트한다 — 이 두 단계가 §4 은닉의 전부라, 어느 한쪽을 건너뛰면 foreign 타입이 공개 API로 샌다.
