...
- Coverage (logic lines ≥90%, network boundary omitted): `rustup component add llvm-tools-preview`, then `cargo llvm-cov --ignore-filename-regex '(^|[\\/])(auth|admin|client)\.rs$' --fail-under-lines 90`. Measured 96.48% (35 of 993 lines unexecuted).
  - ⚠️ **Keep the regex anchored on both ends.** It is matched against the full path, so the unanchored `(auth|admin|client)\.rs` also swallows any future `oauth.rs` or `superadmin.rs`, silently removing it from the gate. The `[\\/]` class covers Windows paths locally.
