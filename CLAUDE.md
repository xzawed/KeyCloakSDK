...
9개 언어 SDK 모두 `main` 병합 완료. PHP·Python·.NET·Rust·Ruby·Node·**Java** 7개는 첫 RC가 공개 레지스트리에 게시됐다(좌표·버전은 아래 표). 나머지 2개(Go·Kotlin)는 미게시이며, 배포는 여전히 전부 사람 승인 게이트다(사람이 태그를 민다).
...
| 언어 | 배포명 | 태그 접두 | 배포 |
|---|---|---|---|
| Java | `io.github.xzawed:keycloak-sdk` | `v*` | 게시됨(`0.1.0-RC1` RC) |
...
- **README는 영문 기본 + 한글 미러**: [`README.md`](README.md)(영문, 기본)와 [`README.ko.md`](README.ko.md)(한글)는 **동일 구조의 미러**다 — 한쪽을 고치면 다른 쪽도 함께 갱신해 동기 유지(상단 상호 링크 `English ↔ 한국어`). 둘 다 슬림 랜딩(정적 배지·9언어 표·30초 퀵스타트·보안·상태·링크)이며, 게시가 언어별로 진행 중인 전환기(human-gated, 9개 중 7개만 첫 RC 게시)이므로 **라이브 레지스트리 배지 금지**(정적 배지만 — 오해 방지).
