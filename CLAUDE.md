...
⚠️ **`doc-budget` 래칫이 올라가는 경우는 둘뿐이다** — (1) 증가분이 **기계 검증**(facts/anchors)을 사 올 때, (2) **사람이 이 문서의 역할을 바꿀 때**(그 판정을 앵커 옆 주석에 적는다). 목표 바이트 수는 없다. 그 밖의 산문 추가는 교환이 아니므로 줄이거나 블록 주석으로 남긴다(주입되지 않아 계상되지 않는다).
