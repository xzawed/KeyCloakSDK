# CLAUDE.md
<!-- doc-budget: max-bytes=24992 max-lines=308 -->
<!-- 24990 → 24992 (2026-09-23). 규약 (1) — **함대 정렬 분기가 뒤집혔다.** JVM 짝이
...
<!-- 24663 → 24863 (2026-09-11, +200B). 규약 (1) 의 가장 순한 형태 — **거짓 문장을 지우는**
     ...
     그 분기는 한 번도 실행된 적이 없다), 같은 거짓을 에러 문구도 인쇄하고 있었다. 늘어난
     것은 전부 **실제로 통하는 두 길**이다 — 경위 산문을 git 으로 보내기, 그래도 안 되면
     `BUDGET_RAISE_CAP` 을 같은 PR 에서 올리기(그 diff 가 사람 판정이다). -->
...
- **근거는 그 사실의 소유자가 낸 출력이다** — 자기 행위도, **수량(「N 건」)**도, 외부 도구·레지스트리 동작도. 붙일 출력이 없으면 그 문장을 쓰지 않고, 자매 생태계가 같다고 가정하지 않는다.
- **왕복 해석기(`node -e`·heredoc·`perl`…)에서 근거를 받지 않는다** — 백슬래시·`$` 를 먹어 **편집은 깨지고 측정은 거짓 0 을 낸다**. 편집은 Edit 툴, 측정은 소유자를 실행한다.
...
9개 언어 SDK 모두 `main` 병합 완료. **9개 언어 전부 정식 릴리스를 냈고, 번호는 갈렸다** — JVM 짝이 `1.0.1`, 나머지 일곱이 `1.0.0`이다(아래 표). ⚠️ **함대 번호로 말하지 말 것**(근거: 1.0 기준(`git show d4e8958:docs/superpowers/plans/release-1.0.md`)). 배포는 전부 사람 승인 게이트다(사람이 태그를 민다).
...
| 언어 | 배포명 | 태그 접두 | 배포 |
|---|---|---|---|
| Java | `io.github.xzawed:keycloak-sdk` (Maven Central) | `v*` | 게시됨(`1.0.1`) |
...
| Kotlin | `io.github.xzawed:keycloak-sdk-kotlin` (Maven Central) | `kotlin-v*` | 게시됨(`1.0.1` · 하한 2.2+) |
...
⚠️ **`doc-budget` 래칫이 올라가는 경우는 둘뿐이다** — (1) 증가분이 **검증 가능성**을 사 올 때(기계 검증, 또는 그 주장을 **다시 재는 명령**), (2) **사람이 이 문서의 역할을 바꿀 때**. 판정을 앵커 주석에 `옛값 → 새값` 으로 적고, (1)은 **같은 커밋이 거짓 문장을 지워야** 한다(추가가 아니라 교환). **+300B 초과는 하드 실패다** — `check-docs.mjs` 검사 8b 가 `main` 과 대조해 막고, ⚠️ **앵커 주석에 판정을 적는 것으로는 못 지나간다**(그런 분기가 없다). 길은 둘 — 경위 산문을 커밋·PR 로 보내거나(판정 방법은 깎지 말 것), 그래도 안 들어가면 `BUDGET_RAISE_CAP` 을 같은 PR 에서 올린다(그 diff 가 사람 판정이다). 목표 바이트 수는 없다. 그 밖의 산문 추가는 교환이 아니므로 줄이거나 블록 주석으로 남긴다(주입되지 않아 계상되지 않는다).
