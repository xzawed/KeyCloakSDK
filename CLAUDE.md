...
**가상 사용자 테스트 하네스 8언어 확장 + 종합 검증·스코어링 완료 — `main` 병합됨 (PR #20).** 8개 언어 SDK가 동일 [HTTP 계약](harness/contract/CONTRACT.md)(v2 — auth 확장 4엔드포인트 + admin 5리소스)으로 실제 Keycloak 26.6에 대해 동형 동작하는지 검증하는 하네스([`harness/`](harness/README.md)). 샘플 앱은 각 언어의 관용 프레임워크로 SDK를 소비한다 — go(net/http)·dotnet(ASP.NET Core)·node(Express 5)·python(FastAPI)·java(Spring Boot)·php(Slim 4)·rust(axum)·ruby(Rack/Puma), 호스트 포트 8090~8097(컨테이너 내부는 모두 8090). 레거시 `run.sh`(k6 부하 실측·비교만, `report/RESULTS.md`)에 더해 **종합 검증·스코어링 파이프라인** `verify.sh`가 추가됐다 — `cd harness && ./verify.sh go dotnet node python java php rust ruby`가 언어별로 Keycloak 기동 → 앱 빌드·기동 → **conformance**(`conformance/conformance.mjs`, 계약 준수 assert) → **security**(`security/probe.mjs`, JWT 하드닝 공격 프로브 — alg=none·HS/RS confusion·flood 등) → k6 성능을 실행하고, 전 언어 종료 후 **suites**(`suites/run-suite.sh`, 각 SDK 자체 단위테스트+커버리지+린트를 툴체인 이미지에서 실행)를 집계, 마지막으로 `report/score.mjs`가 **4차원 가중 스코어카드**(기능30%·보안30%·커버리지20%·성능/동형성20%, 등급 A≥90/B≥80/C≥70/D<70)를 `report/SCORECARD.md`로 산출한다(신호는 `report/signals/<lang>.{conformance,security,suite}.json`). 한 언어의 앱 빌드/헬스체크 실패는 `<lang>.error.json`으로 격리되고 나머지 언어는 계속 진행. ⚠️ 성능·동형성 차원은 아직 k6 실측이 미연동(conformance 통과율 근사, `perf: null` — 후속 작업). **⚠️ 앱 빌드 이미지는 Alpine(musl) 베이스**: Debian/glibc 빌드 이미지는 Docker Desktop(Windows) 내장 DNS 프록시가 패키지 레지스트리의 CNAME 체인을 glibc 리졸버에 실패로 돌려줘 `dotnet restore`/`pip install`/Maven·npm 다운로드가 막힌다(musl은 정상, CI 네이티브 Docker도 무해) — 공유 compose 파일엔 하드코딩 IP/`extra_hosts`가 없다. CI([`.github/workflows/harness.yml`](.github/workflows/harness.yml))는 **안 A** — PR/푸시엔 빠른 Go 스모크 게이트(`mvp-go`), 야간(`schedule` 03:00 UTC)·수동(`workflow_dispatch`)엔 5언어 k6 비교(`all-langs`, `timeout-minutes: 40`) + 8언어 종합 검증·스코어링(`score-all`, `timeout-minutes: 60`, `SCORECARD.md`+`signals/` 아티팩트 업로드)를 실행한다.

**설치·동작 검증 하네스(Install-&-Operate) 완료 — `main` 병합됨 (PR #21).** 실배포(공개 레지스트리) 없이 **각 SDK를 "게시된 패키지처럼" Docker 로컬 레지스트리에서 설치하고 실제 Keycloak 26.6에 대해 동작까지 검증**하는 하네스([`harness/install/`](harness/install/README.md)). 기존 하네스가 SDK를 **소스 경로**로 소비하는 것과 달리, 이 하네스는 실 배포 산출물의 **설치 경로**(매니페스트·파일목록·엔트리포인트·메타데이터·의존성 해석)를 검증한다 — 실배포 전 최종 검증. **8/8 언어 로컬 실측 전 셀 GREEN**(artifact→publish→install→quickstart→app-boot + conformance 26/26 + security 9/9). `cd harness/install && ./install-verify.sh [go dotnet node python java php rust ruby]`가 언어별 4단계(Publish 산출물 빌드→로컬 레지스트리 게시 · Install 소스 없는 클린 컨테이너가 실제 설치명령으로 설치 · Operate quickstart 스모크+conformance+security 재실행 · Report `INSTALL-MATRIX.md`)를 부분실패 격리로 실행한다. **최대 재사용·단일 변수**: `harness/apps/<lang>`·conformance·security·realm 그대로 재사용, 의존성 해석 소스만 소스경로→로컬 레지스트리로 교체. **런타임 패턴**(BuildKit이 build-time custom `--network` 미지원): consume 이미지는 파일만, 런타임 엔트리포인트 `consume/<lang>-run.sh`가 install-net에서 install→quickstart→boot 수행, 상태는 호스트 마운트 `/status` 마커로 회수. **하이브리드 로컬 레지스트리**: node=Verdaccio · python=pypiserver · go=file GOPROXY · dotnet=BaGetter · java=nginx 정적 .m2 · ruby=rubygems-generate_index · php=Satis · rust=cargo-local-registry(소비자 설치명령은 실제와 동일, 소스 URL만 로컬). 정확한 명령 권위 소스는 [설치 레시피 리서치 부록](docs/superpowers/specs/2026-07-07-install-recipes-research.md). 착수 전 병렬 딥리서치(8 에이전트)로 레시피 확정, 완료 후 다중에이전트 리뷰(36 에이전트·47 findings) + fix wave(rust `--locked`→generate-lockfile·java `rm -rf`→in-place·이미지 digest 핀·타임아웃·quickstart 409 멱등) 후 재검증 8/8 무회귀. CI `install-all` 잡(야간 03:00 UTC + `workflow_dispatch`·`timeout-minutes: 90`·`INSTALL-MATRIX.md`+`signals/` 아티팩트, PR/푸시엔 미실행). ⚠️ **앱/레지스트리 전 컨테이너 Alpine/musl**(Windows Docker Desktop glibc-DNS 게차 회피). ⚠️ follow-up(비블로킹): rust CI 이중컴파일 시간·wait_healthy 크래시 조기감지·go 공개프록시 폴스루·rust closure Cargo.lock 커밋.
