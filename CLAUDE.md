...
- **테스트 수(Kotlin, main)**: 단위테스트 100개(10 클래스: Config·Errors·Masking·Tokens·Oidc·TokenProvider·Jwt·Auth·admin.AdminBoundary·Client) + Testcontainers 통합테스트(dasniko, 실제 Keycloak 26.6) 1개(E2E `FullFlowIT`) = **총 101개**, 로직 모듈 커버리지 **라인 99.24%/브랜치 85.71% 실측**(게이트 ≥90/≥85, Kover exclude로 네트워크 경계 `AuthClient*`/`admin.*`/`KeycloakClient*` omit) 통과, `ktlintCheck` 무경고·`explicitApi()` 엄격.
