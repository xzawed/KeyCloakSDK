...
- **9번째 언어**: Kotlin 2.2.20 · JDK 21 · 단일 Gradle 모듈 · coroutines(`suspend`+`runInterruptible(Dispatchers.IO)`) · JVM 자매 Java SDK 라이브러리 스택(`keycloak-admin-client` 26.0.10 + `oauth2-oidc-sdk` 11.37.2) 재사용 래핑 + `nimbus-jose-jwt` 자체 JWT 검증 (`main` 병합, PR #23)
...
- **테스트 수(Java)**: 단위테스트 134개(core 38 · auth 46 · admin 44 · keycloak-sdk 6) + 통합테스트(Testcontainers) 6개(SmokeIT 1 · AuthFlowIT 3 · AdminOpsIT 2) = **총 140개**, 커버리지 게이트(로직 모듈 라인 ≥90%/브랜치 ≥85%) 통과. (배포 전 하드닝으로 +17: PR3 admin 전송오류, PR4 nonce, PR5 signature_algorithms config, PR6 만료·issuer·서명변조 부정테스트, PR#71 jwksMinRefetch config + forRealm 단위커버리지)
...
- **테스트 수(Kotlin, main)**: 단위테스트 107개(Config·Errors·Masking·Tokens·Oidc·TokenProvider·Jwt·Auth·admin.AdminBoundary·Client) + Testcontainers 통합테스트(dasniko, 실제 Keycloak 26.6) 1개(E2E `FullFlowIT`) = **총 108개**, 로직 모듈 커버리지 **라인 99%대/브랜치 85%대 실측**(게이트 ≥90/≥85, Kover exclude로 네트워크 경계 `AuthClient*`/`admin.*`/`KeycloakClient*` omit) 통과, `ktlintCheck` 무경고·`explicitApi()` 엄격. (하드닝으로 +7: PR5 signatureAlgorithms config, PR6 forRealm 실 JWKS 네트워크 경로 부정테스트, PR#71 jwksMinRefetch config)
...
- ⚠️ **(Kotlin) 신규 라이브러리 리스크 0.** JVM 자매 Java SDK가 실 Keycloak으로 이미 필드 검증한 3개 라이브러리(`org.keycloak:keycloak-admin-client` 26.0.10·`com.nimbusds:oauth2-oidc-sdk` 11.37.2·`com.nimbusds:nimbus-jose-jwt` 10.9.1)를 그대로 재사용하므로, Java의 게차(admin-client 버전≠서버 버전·admin 타임아웃 주입·jackson-databind CVE 등)를 코루틴 경계만 다르게 그대로 상속한다.
...
| 의존성 | 좌표 | 버전 |
|---|---|---|
| Admin(재사용, `api`) | `org.keycloak:keycloak-admin-client` | 26.0.10 |
