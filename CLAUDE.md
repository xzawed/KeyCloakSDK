...
- **테스트 수(Java)**: 단위테스트 132개(core 37 · auth 45 · admin 44 · keycloak-sdk 6) + 통합테스트(Testcontainers) 6개(SmokeIT 1 · AuthFlowIT 3 · AdminOpsIT 2) = **총 138개**, 커버리지 게이트(로직 모듈 라인 ≥90%/브랜치 ≥85%) 통과. (배포 전 하드닝으로 +15: PR3 admin 전송오류, PR4 nonce, PR5 signature_algorithms config, PR6 만료·issuer·서명변조 부정테스트)
- **테스트 수(Python, main)**: 단위테스트 236개(sync + `aio` async) + 통합테스트(Testcontainers, 실제 Keycloak 26.6) 11개(sync 6 + async 5) = **총 247개**, 로직 모듈 커버리지 **100% 강제**(`--cov-fail-under=100`, 경계모듈 omit), `mypy --strict`·`ruff`(보안 S/bandit 포함 확장 룰셋)·`ruff format` 통과. (하드닝으로 +12: PR1 exp 필수, PR4 nonce, PR5 signature_algorithms + admin timeout 정정)
- **테스트 수(Node, main)**: 단위테스트 76개 + 통합테스트(Testcontainers, 실제 Keycloak 26.6) 5개(E2E) = **총 81개**, 커버리지 게이트(로직 모듈 라인 ≥90%/브랜치 ≥85% — 네트워크 경계 `auth.ts`/`admin/**`/`index.ts` omit) 통과, `tsc`(strict)·`eslint`·`prettier --check` 통과. (하드닝으로 +5: PR1 exp 필수, PR3 전송오류 변환, PR5 signatureAlgorithms config)
- **테스트 수(Go, main)**: 단위테스트 45개(config·errors·masking·tokens·tokenprovider·oidc·jwt·auth·admin·client) + 통합테스트(Testcontainers, 실제 Keycloak 26.6) 1개(E2E — client-credentials→validate→introspect→user CRUD→5 리소스 CRUD→Raw) = **총 46개**, 커버리지 게이트(로직 파일 statement ≥90% — 실측 95%대, 네트워크 경계 `auth.go`/`admin*.go`/`client.go` omit) 통과, `go vet`·`gofmt`·staticcheck·gosec 통과. (하드닝으로 +5: PR4 nonce, PR5 음수타임아웃 거부·SignatureAlgorithms·ConnectTimeout 배선, PR6 ExpiresIn 파생).
...
- **테스트 수(PHP, main)**: 단위테스트 68개 + 통합테스트(docker CLI 셸아웃, 실제 Keycloak 26.6) 3개(`FullFlowIT`: `testFullFlow`·`testAdminClientCrud`·`testRawEscapeHatch`) = **총 71개**, 집계 로직 라인 커버리지 **100.00%**(게이트 ≥90%, `phpunit.xml` source exclude로 네트워크 경계 `AuthClient`/`Admin/**`/`KeycloakClient` omit) 통과, `phpstan analyse`(level max)·`php-cs-fixer --dry-run --allow-risky=yes`·`composer audit` 통과. (하드닝으로 +4: PR2 JWKS stamp 순서, PR5 signatureAlgorithms config)
- **테스트 수(Rust, main)**: 단위테스트 40개(config · error · tokens · oidc · token_provider · jwks · jwt · auth · admin · client) + 통합테스트(Testcontainers, 실제 Keycloak 26.6) 1개(E2E `full_flow`, `#[ignore]`) = **총 41개**, 로직 모듈 라인 커버리지 **95%대 실측**(게이트 ≥90%, `--ignore-filename-regex '(auth|admin|client)\.rs'`로 네트워크 경계 omit) 통과, `cargo clippy --all-targets -- -D warnings`(0 경고)·`cargo fmt --all --check`·`cargo audit`(RUSTSEC-2023-0071 문서화 예외) 통과. (하드닝으로 +6: PR4 nonce+ValidatedToken.claims, PR5 signature_algorithms config)
- **테스트 수(Ruby, main)**: 단위테스트 83개 + 통합테스트(docker CLI 셸아웃, 실제 Keycloak 26.6) 1개(E2E `full_flow`) = **총 84개**, 로직 모듈 커버리지 **라인 100.0%/브랜치 93%대 실측**(게이트 ≥90/≥85, SimpleCov `add_filter`로 네트워크 경계 `auth_client.rb`/`admin/**`/`client.rb` omit) 통과, `rubocop` 무경고·`bundler-audit check --update` 통과. (하드닝으로 +10: PR4 nonce, PR5 signature_algorithms·rack-oauth2 전역타임아웃 config화, PR6 PKCE S256 파생 검증)
- **테스트 수(Kotlin, main)**: 단위테스트 105개(Config·Errors·Masking·Tokens·Oidc·TokenProvider·Jwt·Auth·admin.AdminBoundary·Client) + Testcontainers 통합테스트(dasniko, 실제 Keycloak 26.6) 1개(E2E `FullFlowIT`) = **총 106개**, 로직 모듈 커버리지 **라인 99%대/브랜치 85%대 실측**(게이트 ≥90/≥85, Kover exclude로 네트워크 경계 `AuthClient*`/`admin.*`/`KeycloakClient*` omit) 통과, `ktlintCheck` 무경고·`explicitApi()` 엄격. (하드닝으로 +5: PR5 signatureAlgorithms config, PR6 forRealm 실 JWKS 네트워크 경로 부정테스트)
