...
- **테스트 수(Ruby, feature/ruby-sdk)**: 단위테스트 73개 + 통합테스트(docker CLI 셸아웃, 실제 Keycloak 26.6) 1개(E2E `full_flow`) = **총 74개**, 로직 모듈 커버리지 **라인 100.0%/브랜치 93.48% 실측**(게이트 ≥90/≥85, SimpleCov `add_filter`로 네트워크 경계 `auth_client.rb`/`admin/**`/`client.rb` omit) 통과, `rubocop` 무경고·`bundler-audit check --update` 통과.
