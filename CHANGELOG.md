...
- **(Java · Kotlin)** 형식이 틀린 설정값이 첫 호출에서 하위 예외로 샜습니다(상대 `serverUrl` 은 Nimbus `SerializeException` 자체였습니다) — 이제 설정을 만들 때 `KeycloakConfigException` 입니다: `serverUrl` 은 절대 http(s) URL, 연결·읽기 타임아웃은 1 ms 이상 int 밀리초 이하입니다(**0 은 무한 대기라 이제 거부합니다**). realm 은 엔드포인트 URL 에서 퍼센트 인코딩되고, `openid` 가 없는 scopes 는 인가 요청에 그대로 실립니다 — 나머지 일곱과 같고, 전에는 Nimbus 예외가 났습니다. (#588)
