...
- **Kotlin 2.2+** on **JDK 21+** — this is the floor of the **published** `1.0.0` (`kotlin-v1.0.0` carries only `jvmToolchain(21)`, no `jvmTarget`). ⚠️ The tree now targets 17, but that has **not shipped**; do not lower this line until a JVM release publishes it ([detail](../docs/guides/getting-started.md#kotlin)). The SDK is built with Kotlin 2.4.20 but pins `languageVersion`/`apiVersion` to 2.2, so its published metadata is consumable by any Kotlin 2.2+ compiler.
