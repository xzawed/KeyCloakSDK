...
Masking covers this SDK's own `toString()`. ⚠️ It does **not** cover serialization: these types carry no serialization hook (no `@Serializable`, no custom `KSerializer`), so kotlinx.serialization, Jackson or Gson reading their properties emits the raw value — [SECURITY.md](../SECURITY.md) lists exactly that as a measured gap, and this line used to claim the opposite. Nor can masking cover what your logging framework, a debugger, or a heap dump does with a value you hand it. The `CharArray` secret is defence in depth rather than an erasure guarantee — `keycloak-admin-client` and Nimbus both take `String`, so the secret is copied into an unerasable heap string at the point of use.
