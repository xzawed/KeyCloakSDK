...
>
> ⚠️ **아홉 언어 전부 정식 릴리스를 냈고, 번호는 갈렸습니다** — Go · PHP `1.2.0`, Ruby `1.1.0`, Rust `1.1.1`, Java · Kotlin `1.0.3`, Python · .NET · Node `1.0.2`입니다. 같은 보증을 함께 갖췄기에 1.0에 함께 도달했습니다 — 공개 API를 깨려면 **major**가 필요하고, CI가 각 레인의 API를 **직전 게시본**과 대조해 그것을 강제합니다. **함대로 움직이지 않습니다**: 소비자에게 보이는 변경이 있은 언어만 번호가 올라갑니다. 모든 배포는 사람 승인 게이트입니다. 설치 명령은 [설치](#설치)에, 붙일 임시 서버는 [지금 바로 써보기](#지금-바로-써보기)에 있습니다.
