#!/usr/bin/env bash
# harness/suites/go.sh — Go SDK 자체 단위테스트+커버리지+린트를 golang:1.26-alpine 컨테이너에서
...
RAW=$(docker run --rm -v "$ROOT/go:/src-ro:ro" -e GOFLAGS=-mod=mod golang:1.26-alpine sh -c '
  set -e
...
if [ "${SUITE_INTEGRATION:-0}" = "1" ]; then
  ...
  IRAW=$(docker run --rm -v "$ROOT/go:/src-ro:ro" -v /var/run/docker.sock:/var/run/docker.sock \
    -e GOFLAGS=-mod=mod golang:1.26-alpine sh -c "cp -r /src-ro /src && cd /src && find . -name '*.go' -exec sed -i 's/\r\$//' {} + && go test -tags=integration -run TestE2E -count=1 -v ./... 2>&1" 2>&1 || true)
