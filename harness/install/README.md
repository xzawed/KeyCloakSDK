...
```bash
cd harness/install
./install-verify.sh                                  # all 9 languages; version per language (see below)
...
**A single language's failure is isolated** — it is recorded as `error` in that language's signal and the remaining languages still run. Isolation is not tolerance, though: `install-verify.sh` judges the matrix with `--strict` at the end and **exits non-zero if any language's row is ✗**, so a failing language turns the job RED (the matrix is the result carrier, not the verdict). This matters beyond the nightly run — `install-smoke` is a pre-publish gate for all nine release workflows.
...
⚠️ **Provenance is asserted, not assumed** (issue #167). Five languages (python · java · kotlin · php · go) *add* the local source while leaving the public registry enabled, because pinning every transitive dependency to the local registry would stop testing the real consumer path. That means a package with the same coordinate **and the same version** on the public registry can satisfy the install — so a broken local artifact would still go green, and the harness would silently be verifying the public package instead of the one just built. Measured, not hypothesized: with the local index unreachable, `pip install "keycloak-sdk==0.1.0rc1"` succeeds from `files.pythonhosted.org` and exits 0. Each of those five `*-run.sh` scripts therefore records the actual download source to `/status/provenance.txt` (pip `--report` · Maven `_remote.repositories` · Gradle `--info` · `gem install -V` · `composer.lock` `dist.url` · file-GOPROXY presence) and **only writes `installed.ok` when it points at the local registry**. The other four (node · rust · dotnet · ruby) also isolate structurally (Verdaccio scope without uplink · cargo source replacement · NuGet `packageSourceMapping` · `gem install --clear-sources`, which drops the public source for the SDK coordinate and installs its runtime dependencies afterwards) — but **structural isolation is not a substitute for the assertion, and node and rust carry both**. ⚠️ **Keep the two axes apart**: the isolation model is 5 add-source / 4 structural, while provenance is recorded and asserted by **all nine** legs (npm `--json` `resolved` · dotnet `.nupkg.metadata` `source` · ⚠️ **rust records cargo's log lines**, not `cargo metadata` — under source replacement that field reports the crates.io index whether or not the crate came from there, so it was rejected as the observation point). Dotnet has a `PROVENANCE_OK` path too: the observation point is the `source` field of `.nupkg.metadata`, and `$REG` is the full index URL, so the gate compares by **exact match** (not the origin-prefix match the other languages use). `scripts/test/test-harness-registries.sh` guards that both the record and the assertion stay in place for all nine (`assert_eq "9" "$prov_langs"`), and it also rejects the shapes that would make the gate vacuous.

## Per-language local registry (hybrid = ecosystem-native local)

`$PKG_VER` below is the version under verification. ⚠️ **There is no single default.** With neither `--version` nor `PKG_VER` set, the version is derived **per language** from that language's manifest (`node scripts/check-versions.mjs --list`, fail-closed); only `go`, `php` and `java` fall back to `0.1.0`, because their published version comes from the tag or is injected at build time (`ver_for_lang` in `lib/verify-lib.sh` owns this). Every command pins it
...
| ruby | static gem repo (generate_index) | `gem install keycloak-sdk --version $PKG_VER --clear-sources --source … --ignore-dependencies` (deps installed afterwards from the public source) |
...
Host ports: node 18090 · python 18091 · go 18092 · dotnet 18093 · java 18094 · ruby 18095 · php 18096 · rust 18097 · kotlin 18098 (for polling the app's healthz). go and rust mount a directory volume into the consume container instead of running a registry service.
