...
<!-- 79274 → 79510 (2026-09-06): 래칫 조건 (1) — **정확성 수정이 판정 방법을 사 온다.** §4 의 PR
     ...
     그 파일이 없었다 — 버전만 범프해 머지하면 **태그도 에러도 없다**. 그 한 줄이 순증분이다.
     같은 커밋에서 §5 의 거짓 주장 둘은 **지웠다**(npm OIDC 미증명 · 「Node and Ruby remain」) —
     경위는 커밋 메시지에 있다. 순증 236B.
...
> ⚠️ **PHP does not publish from this repository, and it never could.** `php-release.yml` runs `git subtree split --prefix=php` into the mirror **`xzawed/keycloak-sdk-php`**. That setup is done (`php-v0.1.0-rc.1` populated the mirror; Packagist is registered). Why a webhook was never possible, and the one-time steps: §2-D.
...
Tier-1 pre-flight (no public registry): run the language's §3 dry-run. `./scripts/release-trigger.sh <lang> <ver>` prints the command. `cargo publish --dry-run --locked` is the only dry-run of the nine that is actually registry-equivalent (it packages *and* compiles the packaged tree). Name-collision checks belong in §5, not in a snapshot of who was unregistered on a given day.

⚠️ **The list of things only a real publish can prove is now empty** — the last two closed on the `0.1.0` round. **npm's OIDC claim matching**: run `32030769342` (2026-08-17) published `+ @xzawed/keycloak-sdk@0.1.0` from a workflow that carries no `NODE_AUTH_TOKEN` at all, and signed a provenance statement into the transparency log (`logIndex=2498394628`). **`sum.golang.org` inclusion**: `lookup/github.com/xzawed/!key!cloak!s!d!k/go@v0.1.0` returns the `h1:` hash, and the proxy lists `v0.1.0`. Everything else was already proven by a real release: PyPI's OIDC exchange (`py-v0.1.0rc1`), the NuGet token push (`dotnet-v0.1.0-rc.1`), Packagist's VCS driver reading the mirror (`php-v0.1.0-rc.1`), `cargo publish` (`rust-v0.1.0-rc.1`, on the third attempt), **RubyGems OIDC** (`ruby-v0.1.0.rc1` — the only workflow with an `environment:`), **npm provenance attestation** (`node-v0.1.0-rc.2` carries a SLSA v1 predicate with a sigstore bundle and transparency-log entries, confirmed from the registry's attestations endpoint), **GPG signing plus the full Central Portal path** (`v0.1.0-RC1`, staging *and* the human Publish — signatures verify against the key on `keyserver.ubuntu.com`), and **Kotlin's vanniktech `publishToMavenCentral` upload** (`kotlin-v0.1.0-RC1` — a different plugin reaching the same registry as Java's Maven path).

Consumption was checked from outside the pipeline: a clean container installed `xzawed/keycloak-sdk:0.1.0-rc.1` from Packagist; `cargo add keycloak-sdk` resolved `0.1.0-rc.1` and compiled against the facade plus the three re-exports (`keycloak_sdk::types`, `KeycloakAdmin<SdkTokenSupplier>`, `keycloak_sdk::reqwest`); all six Java modules resolve on `repo1.maven.org` (BOM as pom-only), `maven-metadata.xml` lists `0.1.0-RC1` as both `latest` and `release`, and `mvn dependency:get` pulled the facade plus `core`/`auth`/`admin`.

⚠️ **npm's entry stayed on that list long after npm had published, for a reason worth keeping**: `node-v0.1.0-rc.2` went out on the temporary `NPM_TOKEN`, and the trusted-publisher relationship was registered *afterwards* (the token and its secret are now deleted, §2-B) — so "npm has published" was true while "OIDC works here" was still unknown. `0.1.0` is what separated them. **The general rule survives its last instance: a mechanism is proven by the run that exercised *it*, not by the registry showing a version.** Before writing that something is proven, name the run and the line in its log.
...
1. **Pre-register a Pending Publisher (Python and Ruby only — one-time, no secret needed)**: register in the registry's Publishing settings **before** pushing the tag. Values are **owner=`xzawed`** · **repo=`KeyCloakSDK`**, workflow filename per language:
   - Python: `python-release.yml` — no expiry is documented for a pending publisher, so this can be registered well in advance. *(Done — and exercised: the `py-v0.1.0rc1` release published over OIDC, converting the pending publisher into a normal one. Ruby did the same on `ruby-v0.1.0.rc1` and Node's was registered after its bootstrap — **none outstanding**.)*
...
2. **Node.** The bootstrap below has been run, and `npm trust list @xzawed/keycloak-sdk` is what confirms it — it should report `type: github · file: node-release.yml · repository: xzawed/KeyCloakSDK · permissions: publish`. The temporary secret and the `NODE_AUTH_TOKEN` mapping are gone, so Node now publishes with **no stored credential**. ⚠️ **Two things cost real attempts here — read them before doing the same for another registry.** (a) The token's *kind* is load-bearing: a granular token **without** bypass-2FA authenticates fine and then fails the `PUT` with `403`, which spent `node-v0.1.0-rc.1`. (b) `npm trust` is an **account-governance** action, so the npm account must have **2FA enabled** — with 2FA off it does not even prompt for an OTP, it returns `403 POST /-/package/…/trust`, and a bypass-2FA token does not help (npm: *"Starting August 2026, the Bypass 2FA setting does not apply to account-identity or account-governance actions"*). The original procedure follows, kept for the next npm package.

   **Node needs one bootstrap publish, and the safe way to do it is through CI.** Because `npm trust` requires the package to exist, the OIDC path cannot make the *first* publish. ⚠️ **Do not learn this by trying it.** A `node-v*` tag pushed before step 1 below reaches the publish step and fails authentication — the npm coordinate survives (npm never receives the tarball), **but the git tag does not**: `RELEASE-TAGS-IMMUTABLE` blocks update and deletion for all nine prefixes, so that version number is spent and you continue at `-rc.2`. Do step 1 first and the first tag succeeds.
   ...
   1. Add a temporary `NPM_TOKEN` secret and map it as `NODE_AUTH_TOKEN` on the existing publish step in `node-release.yml`. ⚠️ **It must be a granular access token with bypass-2FA enabled, and it must be granted by *scope* (`@xzawed`), not by package name** — the package does not exist yet, so it cannot appear in a per-package picker. A token that authenticates but cannot bypass 2FA still fails at the `PUT` with `403 — Two-factor authentication or granular access token with bypass 2fa enabled is required`; that is what spent `node-v0.1.0-rc.1` (§7). npm is also restricting the older classic bypass-2FA tokens, so do not reach for `Automation`.
   2. Push the RC tag (`node-v0.1.0-rc.1`) through the normal pipeline.
   3. `npm trust github @xzawed/keycloak-sdk --repo xzawed/KeyCloakSDK --file node-release.yml --allow-publish` — requires **npm ≥ 11.15.0** (higher than the version needed for trusted publishing itself) and **account-level 2FA**; granular tokens with the bypass-2FA option are rejected. Do **not** pass `--env`: `node-release.yml` declares no `environment:` (only `ruby-release.yml` does), and specifying one that the workflow does not use makes the OIDC claim mismatch. ⚠️ If you get `npm error code EUSAGE / Unknown flag: --allow-publish`, **upgrade npm — do not drop the flag.** Measured: the flag does not exist before 11.15.0, but `npm trust github` itself does, so on an older client the command minus the flag *succeeds* and quietly creates a trust relationship that was never granted publish. Run `npm install -g npm@^11.15.0` first.
...
2. Register it in GitHub Secrets:
   - .NET: `NUGET_API_KEY` — exercised for real by the `dotnet-v0.1.0-rc.1` release
...
This is what makes the "merge a release PR" path of §1 work. **Whether the four steps below are currently applied is answered by `node scripts/repo-config.mjs check`** — read it from the command, not from a date written here. The steps are kept below because they are the recovery procedure, not a to-do list: if a secret is deleted or a ruleset is deactivated, `dispatch-release.yml` fails closed again — it names the missing secret, or reports that the tag rulesets are absent or not yet applied — and no tag is created. Nothing is published and nothing is burned; you simply fall back to pushing the tag by hand, which is the only supported path for Go anyway.
...
3. **Apply the tag rulesets**: `node scripts/repo-config.mjs apply` (needs an admin token). Confirm with `node scripts/repo-config.mjs check` — that command, not a date in this file, is what says whether they are live (do not paste the numeric IDs here; they are server-assigned). The open question this step carried — whether a `target: "tag"` ruleset returns server-managed fields the checker only knew about for branch rulesets — **resolved as "no"**: `check` passed immediately after `apply` with no `SERVER_FIELDS` extension needed. Nothing further is required here.
...
- dry-run: `cd rust && cargo build --locked --all-targets && cargo test --locked && cargo clippy --all-targets -- -D warnings && cargo fmt --all --check && cargo publish --dry-run --locked`
  > `rust/Cargo.lock` is committed, so `--locked` verifies exactly the dependency graph that will be built. `cargo publish --dry-run` is the only way to see the file list that would actually be uploaded (the crate now declares an `exclude` for `tests/`) and to validate the packaging metadata — a plain `cargo build` does not check any of it. Judge that list by **kind**, not by count: `tests/` must be absent (`rust/Cargo.toml` `exclude = ["/tests", "/rustfmt.toml"]`). Do not pin a file count — it has no guard and goes stale when any source file is added.
  >
  > The `75 files, 38.7 kB` comment on `npm pack --dry-run` in [`docs/guides/getting-started.md`](docs/guides/getting-started.md) (Node section) is the same *class* of unguarded snapshot. It stays: that fence is a consumer-tutorial example of expected output, already paired with the kind-check (`dist/` plus `README.md` and `LICENSE`), and the operator-facing criterion is §3 Node above. Restoring cargo's old "19 files packaged" as a pass/fail baseline would have been the opposite role.
...
- dry-run:
  ```bash
  export JAVA_HOME="${KCSDK_JDK21:?see \`node scripts/doctor.mjs java\`}" PATH="${KCSDK_TOOLS:-$HOME/tools}/apache-maven-3.9.9/bin:$PATH"
...
- dry-run:
  ```bash
  export JAVA_HOME="${KCSDK_JDK21:?see \`node scripts/doctor.mjs kotlin\`}" GRADLE_USER_HOME="${GRADLE_USER_HOME:-$HOME/.gradle}"
...
- Install: `go get github.com/xzawed/KeyCloakSDK/go@v1.0.0`
...
5. **A human approves** — either merge the release PR (step 1 is in the PR; steps 2–4 still happen
   ...
   > ⚠️ **Merging is not the trigger — `.github/release-request.json` is** (§1; validate it with
   > `sh scripts/release-request.sh`). A PR that only bumps the version cuts no tag and no error.
...
8. **Verify on the registry itself** — open the package page and confirm the version, the README rendering, and the file list. This is the only step that actually proves the release happened. `node scripts/check-registry-truth.mjs --lang=<lang>` answers the version half: `LIVE` means the registry serves it; `PENDING` means not yet, still inside the propagation grace (72h for Maven, 24h elsewhere) — do not read a first 404 as failure.
...
- **⚠️ What "the release workflows work" does and does not mean.** This document and `scripts/release-readiness.sh`/`scripts/release-trigger.sh` only guide the nine `.github/workflows/*-release.yml`; they do not modify them. Every language has executed its release workflow end to end at least once — **which channel and which version are live is not stated here** (see the lede: `deploy-facts.sh` owns it). Per-language first-publish evidence (which credential path, which tag) is in §0 "what has and has not been proven"; the two RC burns (crates.io email, npm token *kind*) are in §7. ⚠️ **Two Maven Central timing traps.** Portal staging is not publication — a green workflow means "uploaded", not "released". After the Publish click, propagation is not instant: measured, `repo1` answered **404 on the first check and 200 about three minutes later**, while `search.maven.org` still returned `numFound: 0` well after the artifacts were downloadable. Judge by the Portal's deployment state, then poll `repo1`; never conclude "the publish failed" from one 404. Go proxy warming **is** now evidenced — the `go/v0.1.0-rc.1` run's `install-smoke` job passed, and the proxy serves `@v/v0.1.0-rc.1.info`/`.mod` with `Origin.Hash` equal to the tagged commit. What is **still not** evidenced: `gem push` by stored token (Ruby went OIDC instead). Treat the first tag of any *newly added* language as a first execution, not a regression check. That is why §7 recommends spending a release candidate on it.
...
All nine registries support prerelease versions, and most resolvers exclude them from ordinary version ranges by default. All nine languages have now published (§5), and **every one of them spent its first tag on a release candidate** — `php-v0.1.0-rc.1` · `py-v0.1.0rc1` · `dotnet-v0.1.0-rc.1` · `rust-v0.1.0-rc.1` · `ruby-v0.1.0.rc1` · `node-v0.1.0-rc.2` · `v0.1.0-RC1` · `kotlin-v0.1.0-RC1` · `go/v0.1.0-rc.1`. Keep doing that for any language added later: a first tag is a **first execution of an irreversible pipeline**, so spend a release candidate on it instead of `0.1.0`. If the OIDC claim does not match or the GPG public key was never distributed, you find that out on `0.1.0-rc.1` and `0.1.0` is still yours to use. Two of the nine proved the point the expensive way.
...
⚠️ **And again on the sixth — same class, different registry.** `node-v0.1.0-rc.1` passed version, verify, integration and install-smoke, generated its provenance statement, and was then refused by npm at the `PUT`: `403 — Two-factor authentication or granular access token with bypass 2fa enabled is required to publish packages`. The token in `NPM_TOKEN` authenticated fine; it simply was not a *kind* of token allowed to publish. The npm coordinate survived (nothing was uploaded) and the tag did not, so Node continues at `0.1.0-rc.2`. **Two lessons worth carrying:** the token's *type* is as load-bearing as its validity — for npm that means a **granular access token with bypass-2FA enabled**, and npm now warns that older bypass-2FA classic tokens are being restricted; and this property is invisible to `gh secret list`, `release-readiness.sh`, and every local gate, so like the Rust case the only proof is a publish.
...
| Node | `0.1.0-rc.2` (SemVer) | `node-v0.1.0-rc.2` | `^0.1.0` excludes prereleases — see the dist-tag note below |
...
⚠️⚠️ **The very first publish of a package gets `latest` anyway — `--tag` does not prevent it.** Measured on the real registry: `node-v0.1.0-rc.2` published with `--tag rc` (workflow log: `publishing 0.1.0-rc.2 under dist-tag 'rc'`) and npm still produced `dist-tags: {"rc":"0.1.0-rc.2","latest":"0.1.0-rc.2"}`. The `npm-dist-tag` docs do not describe this — they say publishing sets `latest` *unless* `--tag` is used, which is not what happens on a package's first version. The consequence is exactly what §7 exists to prevent: a bare install hands everyone the RC. ⚠️⚠️ **And you cannot undo it.** `npm dist-tag rm @xzawed/keycloak-sdk latest` is refused — `403 Forbidden — DELETE /-/package/@xzawed%2fkeycloak-sdk/dist-tags/latest`, using the very token that had published the package minutes earlier. The `npm-dist-tag` docs say nothing about `latest` being protected; the registry simply is. **So for npm this is not a mistake you fix, it is a state you inherit**: a package whose first release is a prerelease has `latest` pointing at that prerelease until a stable version takes it over, and §7's "an RC must not be the default install" cannot be enforced there. Plan for it — say so in the package README rather than claiming a bare install fails. **Check `npm view <pkg> dist-tags` after any first publish** and do not assume `--tag` was honoured.

⚠️ **What a missing `latest` would do (for reference — it is not this package's situation).** Measured against a synthetic registry serving `dist-tags: {"rc": "0.1.0-rc.1"}` and nothing else, npm does **not** fall back to the prerelease the way pip and Cargo do — it errors:

| consumer form | result |
|---|---|
| `npm install @xzawed/keycloak-sdk` | ❌ `ETARGET — No matching version found for …@*` |
| `"@xzawed/keycloak-sdk": "^0.1.0"` | ❌ `ETARGET` |
