...
All nine language SDKs have a **tag-driven release CI** wired up. Eight tags have been pushed so far — `php-v0.1.0-rc.1` (the deliberate rehearsal described in §0 Step 0), `py-v0.1.0rc1`, `dotnet-v0.1.0-rc.1`, `rust-v0.1.0-rc.1`, `ruby-v0.1.0.rc1`, `node-v0.1.0-rc.2`, `v0.1.0-RC1` (Java), and `kotlin-v0.1.0-RC1` — and eight packages are live on public registries as a result: **`xzawed/keycloak-sdk` v0.1.0-rc.1 on Packagist**, **`keycloak-sdk` 0.1.0rc1 on PyPI**, **`Xzawed.Keycloak.Sdk` 0.1.0-rc.1 on NuGet**, **`keycloak-sdk` 0.1.0-rc.1 on crates.io**, **`keycloak-sdk` 0.1.0.rc1 on RubyGems**, **`@xzawed/keycloak-sdk` 0.1.0-rc.2 on npm**, **`io.github.xzawed:keycloak-sdk` 0.1.0-RC1 on Maven Central**, and **`io.github.xzawed:keycloak-sdk-kotlin` 0.1.0-RC1 on Maven Central**, all prereleases (§0 spells out precisely what those runs did and did not prove). The other one language is unpublished. Actual deployment is a human-gated approval step triggered **only when a human pushes a tag**, and the prerequisites below (accounts, keys, tokens) can only be performed by the repository owner.
...
- **⚠️ What "the release workflows work" does and does not mean.** This document and `scripts/release-readiness.sh`/`scripts/release-trigger.sh` only guide the nine `.github/workflows/*-release.yml`; they do not modify them. **eight of nine languages are published — PHP, Python, .NET, Rust, Ruby, Node, Java and Kotlin, all as prereleases — and one is not.** Eight release workflows have now executed end to end. Per-language first-publish evidence (which credential path, which tag) is in §0 "what has and has not been proven"; the two RC burns (crates.io email, npm token *kind*) are in §7. ⚠️ **Two Maven Central timing traps.** Portal staging is not publication — a green workflow means "uploaded", not "released". After the Publish click, propagation is not instant: measured, `repo1` answered **404 on the first check and 200 about three minutes later**, while `search.maven.org` still returned `numFound: 0` well after the artifacts were downloadable. Judge by the Portal's deployment state, then poll `repo1`; never conclude "the publish failed" from one 404. What is **still not** evidenced: `gem push` by stored token (Ruby went OIDC instead), npm's OIDC claim matching (the one publish so far used a token), and Go proxy warming. Treat each remaining first tag as a first execution, not a regression check. That is why §7 recommends spending a release candidate on it.
...
All nine registries support prerelease versions, and most resolvers exclude them from ordinary version ranges by default. One of the nine languages (Go) has not yet published anything (§5), so its first tag is a **first execution of an irreversible pipeline**. Spend a release candidate on it instead of `0.1.0`: if the OIDC claim does not match or the GPG public key was never distributed, you find that out on `0.1.0-rc.1` and `0.1.0` is still yours to use. The eight languages that have published (PHP · Python · .NET · Rust · Ruby · Node · Java · Kotlin) spent their first tags exactly this way — `php-v0.1.0-rc.1` · `py-v0.1.0rc1` · `dotnet-v0.1.0-rc.1` · `rust-v0.1.0-rc.1` · `ruby-v0.1.0.rc1` · `node-v0.1.0-rc.2` · `v0.1.0-RC1` · `kotlin-v0.1.0-RC1`.
