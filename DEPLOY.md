...
All nine language SDKs have a **tag-driven release CI** wired up. Seven tags have been pushed so far — `php-v0.1.0-rc.1` (the deliberate rehearsal described in §0 Step 0), `py-v0.1.0rc1`, `dotnet-v0.1.0-rc.1`, `rust-v0.1.0-rc.1`, `ruby-v0.1.0.rc1`, `node-v0.1.0-rc.2`, and `v0.1.0-RC1` (Java) — and seven packages are live on public registries as a result: **`xzawed/keycloak-sdk` v0.1.0-rc.1 on Packagist**, **`keycloak-sdk` 0.1.0rc1 on PyPI**, **`Xzawed.Keycloak.Sdk` 0.1.0-rc.1 on NuGet**, **`keycloak-sdk` 0.1.0-rc.1 on crates.io**, **`keycloak-sdk` 0.1.0.rc1 on RubyGems**, **`@xzawed/keycloak-sdk` 0.1.0-rc.2 on npm**, and **`io.github.xzawed:keycloak-sdk` 0.1.0-RC1 on Maven Central**, all prereleases (§5 spells out precisely what those runs did and did not prove). The other two languages are unpublished. Actual deployment is a human-gated approval step triggered **only when a human pushes a tag**, and the prerequisites below (accounts, keys, tokens) can only be performed by the repository owner.
