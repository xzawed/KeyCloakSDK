...
All nine language SDKs have a **tag-driven release CI** wired up. Three tags have been pushed so far — `php-v0.1.0-rc.1` (the deliberate rehearsal described in §0 Step 0), `py-v0.1.0rc1`, and `dotnet-v0.1.0-rc.1` — and three packages are live on public registries as a result: **`xzawed/keycloak-sdk` v0.1.0-rc.1 on Packagist**, **`keycloak-sdk` 0.1.0rc1 on PyPI**, and **`Xzawed.Keycloak.Sdk` 0.1.0-rc.1 on NuGet**, all prereleases (§5 spells out precisely what those runs did and did not prove). The other six languages are unpublished. Actual deployment is a human-gated approval step triggered **only when a human pushes a tag**, and the prerequisites below (accounts, keys, tokens) can only be performed by the repository owner.
...
2. Register it in GitHub Secrets:
   ...
   - Rust: `CARGO_REGISTRY_TOKEN` *(still unset)*
