...
All nine registries support prerelease versions, and most resolvers exclude them from ordinary version ranges by default. Three of the nine languages have not yet published anything (§5), so each of their first tags is a **first execution of an irreversible pipeline**. Spend a release candidate on it instead of `0.1.0`: if the OIDC claim does not match or the GPG public key was never distributed, you find that out on `0.1.0-rc.1` and `0.1.0` is still yours to use. The six languages that have published (PHP · Python · .NET · Rust · Ruby · Node) spent their first tags exactly this way — `php-v0.1.0-rc.1` · `py-v0.1.0rc1` · `dotnet-v0.1.0-rc.1` · `rust-v0.1.0-rc.1` · `ruby-v0.1.0.rc1` · `node-v0.1.0-rc.2`.
