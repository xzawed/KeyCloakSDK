...
All nine language SDKs have a **tag-driven release CI** wired up. Nine tags have been pushed so far — `php-v0.1.0-rc.1` (the deliberate rehearsal described in §0 Step 0), `py-v0.1.0rc1`, `dotnet-v0.1.0-rc.1`, `rust-v0.1.0-rc.1`, `ruby-v0.1.0.rc1`, `node-v0.1.0-rc.2`, `v0.1.0-RC1` (Java), `kotlin-v0.1.0-RC1`, and `go/v0.1.0-rc.1` — and nine packages are live on public registries as a result: **`xzawed/keycloak-sdk` v0.1.0-rc.1 on Packagist**, **`keycloak-sdk` 0.1.0rc1 on PyPI**, **`Xzawed.Keycloak.Sdk` 0.1.0-rc.1 on NuGet**, **`keycloak-sdk` 0.1.0-rc.1 on crates.io**, **`keycloak-sdk` 0.1.0.rc1 on RubyGems**, **`@xzawed/keycloak-sdk` 0.1.0-rc.2 on npm**, **`io.github.xzawed:keycloak-sdk` 0.1.0-RC1 on Maven Central**, **`io.github.xzawed:keycloak-sdk-kotlin` 0.1.0-RC1 on Maven Central**, and **`github.com/xzawed/KeyCloakSDK/go` v0.1.0-rc.1 on the Go module proxy (proxy.golang.org)**, all prereleases (§0 spells out precisely what those runs did and did not prove). Every language has now published its first release candidate; none has a stable release. Actual deployment is a human-gated approval step triggered **only when a human pushes a tag**, and the prerequisites below (accounts, keys, tokens) can only be performed by the repository owner.
...
3. ~~**Apply the tag rulesets**: `node scripts/repo-config.mjs apply` (needs an admin token).~~ **Done — 2026-08-04.** All three are live. Confirm with `node scripts/repo-config.mjs check` (do not paste the numeric IDs here — they are server-assigned). The open question this step carried — whether a `target: "tag"` ruleset returns server-managed fields the checker only knew about for branch rulesets — **resolved as "no"**: `check` passed immediately after `apply` with no `SERVER_FIELDS` extension needed. Nothing further is required here.
