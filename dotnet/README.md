...
> **`1.0.2` is on NuGet** — a patch release of security fixes on top of `1.0.1`; `1.0.0` was the first release carrying the stability guarantee set out under *Versioning and support* below. A bare `dotnet add package Xzawed.Keycloak.Sdk` resolves it. An error raised on a malformed token or introspection response no longer prints that response (present in `1.0.0` and `1.0.1`): through the `InnerException` chain `ToString()` showed a form-encoded body with its live access token, a non-JSON id_token header or payload, a malformed response header line or an unknown `charset`, and the server's reason phrase and a non-string `error` member reached the message and `OAuthError`. ⚠️ **An inner exception chain that quotes response input is now attached as a copy without that input** — `JsonException`, `HttpRequestException` and `HttpIOException` keep their type under a fixed message, the rest of that chain becomes internal stand-ins that keep the type name and stack trace, and a chain raised while reading a response that did arrive keeps no message at all. ⚠️ **A token or introspection response whose JSON root is not an object, or an empty introspection body, is now a `KeycloakAuthException`** instead of an escaping `InvalidOperationException`. No public API changed.
