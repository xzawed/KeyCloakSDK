...
> **`1.0.2` is on PyPI** — a bare `pip install keycloak-sdk` resolves it. A patch release of security fixes (present in `1.0.0` and `1.0.1`): an error raised on a malformed or request-echoing token, introspection or logout response — or on the admin client's token grant — no longer carries that response's tokens or an echoed `client_secret` in its message or cause chain. SDK error messages now hold the HTTP status and, when it is shaped like one, the OAuth error code, and `__cause__` is a summary (the lower exception's type, status and where it was raised) instead of the python-keycloak exception, so code that inspected `err.__cause__` for a `keycloak.exceptions` type no longer finds one. A `TypeError` or `ValueError` that escaped raw is now `KeycloakAuthError` (auth) or `KeycloakTransportError` (admin), and a token or introspection response with a wrongly typed field is rejected. ⚠️ **An introspection `active` that is not a JSON boolean (such as the string `"false"`) now raises `KeycloakAuthError`** — it used to read as **active**. `1.0.0` was the first release carrying the stability guarantee: a breaking change to the public API requires a **major** bump.
>
