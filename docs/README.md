...
|---|---|---|---|
| [1.0 릴리스 기준](superpowers/plans/release-1.0.md) | 2026-08-30 | 완료 | **무엇이 참이어야 1.0 을 붙일 수 있는가**를 항목마다 **돌아가는 명령 + 실측 출력**으로 세운 유일한 자리. 아홉 API 게이트의 **커버리지 차이**(넷은 종료코드가 거짓말해 본문을 근거로 삼고, ruby·node 둘은 못 막는 자리가 있다)와 그 한계를 감추지 않는 문장. 매니페스트 버전과 **게이트 기준선을 같은 커밋에서 올려야 하는 이유**와 그 일곱 자리의 목록. |
