...
태그로 내리기 전에 각각 [작업 프로세스](governance/process.md) §3과 아래 §3으로 옮긴다.
...
|---|---|---|---|
| [미해소 갭](superpowers/plans/open-gaps.md) | 2026-08-23 | 진행 | 문서·가드가 트리와 어긋난 채 **남아 있는 넷**과 각각의 재현 명령. `TokenStore`가 9언어 기본선으로 선언됐지만 Java 전용이고 그 Java조차 프로덕션이 안 쓴다는 실측. python `with`가 admin 세션을 정리한다는 README 문장의 절반이 no-op이라는 것. 커버리지 대조기가 `dotnet/**` 경로에서만 생성된다는 것. 룰셋 App 분할 가드가 비-required 잡에 있다는 것. |
