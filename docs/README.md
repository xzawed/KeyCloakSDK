...
끝난 설계 스펙·WBS·검증 로그는 아카이브 태그에 있다 — `archive/docs-history-2026-08`(설계·WBS 일괄, #191)·
`archive/docs-history-2026-08b`(하네스 판정·출처 완결 계획서, #213)·`archive/docs-history-2026-08c`(문서 IA
재설계 SDD, #295). **기각 항목의 근거는 거기에만 남는다** — 살아 있어야 하는 기각과 **아직 안 닫힌 항목**은
태그로 내리기 전에 각각 [기각 레지스트리](governance/rejected.md)와 아래 §3으로 옮긴다.
⚠️ **`main` 이력에 이미 있는 문서는 태그를 새로 만들지 않는다** — squash로 개별 SHA가 사라진 경우에만 태그가
필요하다. 예: 「미해소 갭」 계획서는 `git show f6061a2:docs/superpowers/plans/open-gaps.md`로 읽는다(#297).
...
| 문서 | 상태 | 여기서만 알 수 있는 것 |
|---|---|---|
| [작업 프로세스](governance/process.md) | 운영 | **모든 작업의 6단계**(기획→계획→검토→일정→수행→검증)와 각 단계의 나가는 조건. **WBS 규약**(상태 표기·계층·"끝나는 조건은 명령이어야 한다"). **PM의 중재 규칙** — 라우팅·Grok 독립 레그·결과 재검증·갈릴 때의 재정. 품질 게이트 G1–G6. **기각 체크리스트 7**(판정 기준 — 내려진 판정 목록은 아래 레지스트리). 변이 3요건 (a)(b)(c)가 각각 무엇을 답하는지도. |
...
|---|---|---|---|
| [1.0 릴리스 기준](superpowers/plans/release-1.0.md) | 2026-08-30 | 진행 | **무엇이 참이어야 1.0 을 붙일 수 있는가**를 항목마다 **돌아가는 명령 + 실측 출력**으로 세운 유일한 자리. 아홉 API 게이트의 **커버리지 차이**(넷은 종료코드가 거짓말해 본문을 근거로 삼고, ruby·node 둘은 못 막는 자리가 있다)와 그 한계를 감추지 않는 문장. 매니페스트 버전과 **게이트 기준선을 같은 커밋에서 올려야 하는 이유**와 그 일곱 자리의 목록. |
