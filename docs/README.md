...
체크박스가 **실제 할 일**인 열린 작업을 둔다. 끝난 계획서는 위 아카이브 태그에 있다.
⚠️ 지금 남은 것은 **완료된 계획서 하나뿐**이다 — 파일을 내릴지, 그리고 이 절 자체를 접을지는 #213이 정한다.

| 문서 | 날짜 | 상태 | 여기서만 알 수 있는 것 |
|---|---|---|---|
| [하네스 판정·출처 완결 계획](superpowers/plans/2026-08-12-harness-judgment-and-provenance-completion.md) | 08-12 | 완료 | 판정·관측·가드 3계층 분해. 미체크 **0건** — Phase A~D는 처리·기각됐고 Phase E 두 게이트도 닫혔다(Go 첫 태그 `go/v0.1.0-rc.1` · 태그 커팅 App `85715ec`). ⚠️ S-B1(dotnet 레그 실패)은 git-ignored 로컬 산출물을 CI 상태로 오독한 것이라 **틀렸다**. 파일을 내릴지는 #213이 정한다. |
