...
- **Path and body stay separate on `update`.** Keycloak renames a resource with `PUT /{current address}` carrying the *new* name in the body. If an implementation derives the path from the representation, the rename cannot be expressed at all. ⚠️ **Two languages needed a departure for this, in two different places.** Go: gocloak builds the path from the representation, so `realms.update` is the one method there that does not delegate to gocloak. PHP: fschmtt's `Roles::update` reads the name from the representation, so `roles.update` goes through `Keycloak::resource()` with the path and body kept apart — which is also why the PHP signature is `update(string $name, Role $role)` like every sister language, rather than fschmtt's single-argument shape.
- **A partial `update` is safe everywhere; what differs is whether you can *clear* a field.** Sending a sparse representation never clobbers the fields you left out — that is measured end-to-end for Kotlin (`FullFlowIT.kt`, "sparse update must not clobber username/email") and Rust (`rust/tests/integration_test.rs`), both against a real 26.6 server. The portability hazard is the other direction. Two shapes:
  - **Typed representation** (Java · Kotlin · Go · Rust · .NET · PHP · Node): the object carries no "explicitly null" state, so an unset field is simply omitted and **you cannot null a field out through `update`**. On Java and Kotlin that is the admin-client's Jackson `NON_NULL` inclusion (`AdminClient.java`, pinned by `AdminJacksonProviderTest`); the others reach the same result through their own omit-empty serialisation. ⚠️ **The mechanism is per-library and only the JVM pair is pinned by a test here** — if you depend on it, assert it on your own server rather than porting this sentence.
  - **Plain map** (Python · Ruby): the SDK passes your `dict`/`Hash` straight through, so exactly the keys you supply are sent — an explicit `None`/`nil` reaches the wire as `null`. That is more expressive and easier to get wrong: a key you did not mean to include is a write.
