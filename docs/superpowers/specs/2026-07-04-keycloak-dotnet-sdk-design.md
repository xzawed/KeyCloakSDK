...
| **DI** | `Microsoft.Extensions.DependencyInjection.Abstractions` | 9.0.8 | MIT | `AddKeycloak` 확장(`IServiceCollection`·`AddSingleton`). **9.0.8 = Keycloak.AuthServices.Sdk 2.7.0의 요구 하한**(낮추면 NU1605; 9.0.x는 net8.0 지원). HttpClient 전략은 §6(단일 장수명 + `PooledConnectionLifetime`) — `IHttpClientFactory` 미사용(스테이트풀 재사용 클라이언트에 부적합, 리뷰 확정). |
