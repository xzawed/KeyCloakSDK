...
- **admin(파사드 + raw())** — `AdminClient`: fschmtt `Keycloak`를 `Builder`로 구성(주입한 Guzzle에 config 타임아웃/TLS 배선), `users()/clients()/realms()/roles()/groups()` 리소스 파사드. **Guzzle 예외를 경계에서 변환**(§4). fschmtt `TokenStorageInterface`에 우리 `ClientCredentialsTokenProvider` 배선(토큰 재사용). `raw()` 탈출구는 fschmtt `Keycloak::resource(커스텀 Resource)` 기반(하위 Guzzle 클라이언트가 private이라 직접 핸들 없음).
