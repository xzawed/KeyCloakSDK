...
- [ ] **P-4 게시 확인 뒤 API 게이트 기준선 7자리를 올린다.** `python-ci.yml`(`--against`)은 **완료** — python 만 `1.0.0` 이 실제 게시돼 선행조건이 이미 참이었다. 남은 여섯: `java/pom.xml`(`japicmp.baseline`) · `kotlin-ci.yml`(`BASELINE`) · `php-ci.yml`(태그) · `ruby-ci.yml`(`BASELINE`) · `node-ci.yml`(`BASELINE`) · `dotnet` csproj(`PackageValidationBaselineVersion`). ⚠️ **게시 전에는 못 올린다** — 없는 버전을 받으러 간다(위 경고). 안 올리면 각 README 의 「직전 게시본과 대조한다」가 다음 릴리스부터 거짓이 된다.
