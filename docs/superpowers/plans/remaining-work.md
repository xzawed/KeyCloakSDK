...
- [ ] `published-bytes-have-no-oracle` **[M/M · 신규 2026-09-06]** 아무 가드도 **게시된 바이트**를 읽지 않는다 — 태그까지가 한계다 · `scripts/check-jvm-bytecode-floor.mjs` · `scripts/check-docs.mjs:검사 8b/kind=runtime`
  ...
  - **(2) 선언 ≠ 방출된 바이트.** `check-jvm-bytecode-floor.mjs` 는 디렉터리를 받는데, **호출 지점 둘 다 로컬 빌드 산출물**이다(실측: `ci.yml:43` → `java/*/target/classes` · `kotlin-ci.yml:36` → `kotlin/build/classes/kotlin/main`). ⚠️ **「어떤 가드도 안 읽는다」로 쓰지 말 것** — 그건 전수 조사가 아니고, 독립 검증 레그가 그 점을 지목했다. 참인 진술은 **「게시된 jar 를 받아 바이트를 읽는 호출 지점이 0개」**다(`scripts/` 에서 `repo1` 을 부르는 것들은 `release-readiness.sh`·`deploy-facts.sh` 처럼 **존재 확인**이지 바이트 검사가 아니다). 위 91/91 측정이 그걸 **사람 손으로 한 번** 했다는 것이 지금의 전부다.
...
- [x] `doc-audit-batch1-remainder` **[H/L · 신규 2026-09-06 · 닫힘 2026-09-06 #416]** 소비자 문서 배치 1(10개) 감사의 **잔여 9건** — 인용 181건 전건 실재(계측기 5/5 자가검증), 반박 10건 중 2건 refuted · `docs/guides/` · `docs/reference/`
  ...
  - ⚠️ **착수 전 필독 — 반박자가 감사자의 수치를 여러 건 정정했다.** 특히 여러 감사자가 처방에 `--min-facts=64 --min-anchors=21` 을 전제로 썼는데 이 트리는 이미 **76/26** 다(`repo-hygiene.yml:87`·`CLAUDE.md:69`). 감사 처방의 숫자를 그대로 옮기지 말 것.
...
- [x] `harness-orphan-container-reads-as-build-failure` **[L/S · 닫힘 2026-09-16 · 부류로 넓혀서]** 중단된 하네스 런이 남긴 컨테이너가 다음 런을 막는데, 신호가 **원인을 잘못 가리킨다** · `harness/verify.sh:30`
  - ✅ **닫힘 2026-09-16 — 이 항목이 말한 것보다 부류가 넓었고, 그 사이 실제 사고가 났다.** 야간 `score-all` 이 **2026-09-08~09-15 여덟 밤** 연속 빨갰다(마지막 초록 09-07). 아티팩트에 남은 것은 `{"name":"token(client-creds)","ok":false,"detail":"500"}` 뿐이었고 **앱이 던진 예외 문장은 어디에도 없었다** — `stop` 뒤 EXIT 트랩의 `down -v` 가 컨테이너를 지운다. 원인을 알아내는 데 로컬 재빌드와 통제 실험이 필요했다. 이 항목은 「이름 충돌 하나」를 적었지만 참인 부류는 **「실패 신호가 증상만 담고 원인을 안 담는다」**다.
  - **사고 자체는 상류가 냈고 상류가 닫았다**(우리 코드 무변경). `json` 3.0.0 게시 **2026-09-07 06:57Z** → 그날 야간(03:14Z)은 그 전이라 초록, 다음 밤부터 ruby 앱이 전 경로 500. `faraday` 2.14.4 게시 **2026-09-15 13:16Z** → **09-16 야간 초록 복귀**(ruby A/90 · 26/26). ⚠️ **gem 집합 실측이 갈랐다**: 09-07(초록) ↔ 09-08(첫 빨강) 차이는 **`json 2.21.2 → 3.0.0` 한 줄**뿐이었다.
  - **통제 실험이 인과를 못박았다**(같은 이미지·같은 코드, faraday 만 교체): `2.14.4` → `RESULT=OK`, `2.14.3` → `RESULT=ERR KeycloakSdk::TransportError: ... wrong number of arguments (given 2, expected 1)` (`auth_client.rb:68`). 즉 faraday 2.14.3 의 json 처리가 json 3 과 안 맞았고 2.14.4 가 고쳤다.
  - ⚠️ **install 하네스는 같은 밤 초록이었다** — 같은 `app.rb` 인데 gem 을 gemspec 으로 설치해 faraday 해석이 달랐다. 「SDK 결함」으로 읽힐 뻔한 자리를 그 대조가 갈랐다.
  - **처방은 `--remove-orphans` 가 아니라 증거다.** 이 항목이 적은 두 처방 중 앞의 것은 **안 골랐다** — `--profile apps` 와 함께 쓰면 비활성 프로필의 형제 컨테이너를 지울 수 있고 그 범위를 이번에 측정하지 않았다. 고른 쪽: `capture_app_log`(루프를 빠져나가는 **세 자리 전부** — build/up 실패·healthz 타임아웃·정상 종료)와 `capture_compose_ps`(실패 두 자리). 둘 다 `report/signals/` 에 쓴다 — 업로드되는 것이 그곳뿐이다.
  - ⚠️ **빈 로그를 증거로 착각하지 않는다**(독립 레그가 지목한 침묵 실패): 캡처가 비면 「빈 로그 — stdout 에 아무것도 안 썼다」를 **적는다**. 안 그러면 빈 파일이 「증거 없음」과 구분되지 않는다.
  - **⚠️ 각주(8080 못박힘)도 함께 닫았다 — 이번에 실제로 막혔기 때문이다.** 다른 프로세스가 8080 을 쥔 PC 에서 `Bind for 127.0.0.1:8080 failed: port is already allocated` 로 **기동조차 못 했다**(실측). compose 의 호스트 퍼블리시 **10개 전부**를 `${HARNESS_KC_PORT:-8080}` · `${HARNESS_PORT_<LANG>:-809N}` 로 뺐다(기본값 보존 = 동작 무변경). 스크립트는 이미 `docker compose port` 로 읽고 있어 손댈 것이 없었다.
  - **가드 둘** (repo-hygiene 배선): `test-harness-failure-evidence.sh`(7 단언 — 캡처 존재·호출 3자리·stop 앞 순서·빈 로그 마커·ps 덤프·공허 하한 4) · `test-harness-host-ports.sh`(5 단언 — 변수화·공허 하한 10·기본값 고유·변수명 고유).
...
- [ ] `go-consumer-floor-1-26-decision` **[M/S · 신규 2026-09-11]** `golang.org/x/oauth2 v0.37.0` 과 `golang.org/x/sync v0.23.0` 이 둘 다 `go 1.26.0` 을 선언해, 그 둘을 받으면 **소비자 하한이 1.25 → 1.26 으로 올라간다**(`go get` 이 스스로 `go.mod` 의 `go` 지시자를 올린다 — 실측). #453 이 그래서 실패했고 닫았다. ⚠️ **dependabot 잘못이 아니다** — 상류가 하한을 올린 것이고, dependabot 은 「하한을 안 올리는 선에서만」을 표현할 수 없다. 그래서 이 PR 은 하한을 안 건드리는 `go-jose/v4 4.1.5`(그 자신은 `go 1.24.0`) 만 받았다. **판정이 필요한 것**: 1.26 으로 올릴 것인가(소비자 절단 + 릴리스 노트), 아니면 두 패키지를 현재 버전에 묶어 둘 것인가. 후자를 고르면 `.github/dependabot.yml` 에 `ignore` 와 **되살릴 조건**을 함께 적어야 한다 — 지금은 매주 같은 PR 이 같은 이유로 빨개진다. 재현: `cd go && go get golang.org/x/oauth2@v0.37.0 && grep '^go ' go.mod`
...
- [x] `stale-release-comments` **[H/S · 닫힘 2026-09-09 #447]** 릴리스 경로의 주석 **여섯**이 낡았고, 그중 하나는 다음 릴리스를 정반대로 오도했다 · `.github/workflows/install-smoke.yml:57`
  ...
  - **caller 다섯**: 「`version` 을 안 넘기면 게이트가 기본 버전을 검증한다」 — 그 입력은 `required: true` 에 기본값이 없어 **호출 자체가 실패한다**(그 파일이 왜 기본값을 안 두는지 직접 설명한다).
  - **락스텝 문단**: 「즉 락스텝 정책을 강제한다 … 전환하는 날 정리해야 한다」 — `check-versions.mjs` 는 언어 간 기저 버전 갈림을 **경고로 밀고 exit 0** 이고 자기 요약이 「차단하지 않는다」라고 쓴다. 전환은 이미 일어났다. ⚠️ **두 방향으로 틀린다** — 독립 릴리스를 금지로 읽거나, 하드 실패를 **되살려** 실제로 막는다.
  - 가드는 규칙 7c(`test-selftest-hygiene.sh`)이고 **양방향 파생**이다: 기본값이 다시 생기거나 갈림이 다시 error 가 되면 그 문장이 참이 되어 통과한다.
  - ⚠️ 스텝 이름의 「반쯤 적용된 범프」는 **여전히 참**이라 그대로 뒀다 — 한 언어 안의 POM 갈림은 지금도 error 다.
