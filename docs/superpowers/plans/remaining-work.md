...
- [ ] `seven-selftests-have-no-negative-control` **[H/L · 계수 정정 2026-09-09]** **여섯** 자가테스트가 라이브 상태만 단언한다 — 검출기를 지워도 통과한다 · `scripts/test/test-deploy-md.sh:7`
  ...
  - ⏸ **③ 이 남았다**(`SILENT` 로 실증): `test-security-defaults.sh` 의 python 마스킹 카나리아는 `test_repr_masks` 를 지워도 통과한다 — 게시된 SDK 가 토큰을 로그에 찍는 부류다. ⚠️ 그 축은 **테스트 파일에 `***` 가 있는가**를 보는데, 같은 파일의 다른 테스트(`mask()` 단위 테스트)가 그 문자열을 갖고 있어 카나리아가 사라져도 참이다.
