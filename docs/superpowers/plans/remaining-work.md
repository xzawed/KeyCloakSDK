...
2. **A 절 잔여 1건**(`auto-bump-manifest-crosscheck-skip` [L·보류]) — 확정 결함 12건 중 남은 전부다(`java-rules-close-scope-ambiguous` 는 #422 에서 닫혔다).
...
- [x] `java-rules-close-scope-ambiguous` **[L/S · 닫힘 2026-09-06 #422]** [weak·채택] .claude/rules/java.md가 close()의 정리 범위를 java/README.md와 반대로 읽히게 적는다 · `.claude/rules/java.md:33`
...
- [ ] `doc-audit-batch2-3-not-started` **[M/L · 신규 2026-09-06 · 배치 2 닫힘 2026-09-06 #421]** 문서 41개 중 20개가 미검증이었다 — **배치 2(언어별 README 9 + `language-support.md`) 완료, 배치 3(하네스·내부 7 + rules 3 재검증) 10개가 남았다**
  ...
  - **#422 가 고친 것은 rules 3 + 그 파생 3자리뿐**(`kotlin.md`·`DEPLOY.md` 둘·`development-setup.md`). **나머지 7문서 37건은 열려 있다** — 아래 `doc-audit-batch3-fixes-outstanding`.
...
- [ ] `doc-audit-batch3-fixes-outstanding` **[M/M · 신규 2026-09-06]** 배치 3 발견 50건 중 **37건 미수정** — 하네스 3 · 문서 지도 · `process.md` · `rejected.md` · 플레이북 · `CONTRACT.md` · `docs/superpowers/plans/remaining-work.md` (37건)
  ...
  - ⚠️ **예산이 수정을 막는다** — `process.md` 여유 **12B**, `rejected.md` 는 상한 근처다. 검사 8b 가 **+300B 초과를 하드 실패**시키므로(앵커 주석만으로는 못 지나간다) 인상 전에 압축할 자리를 먼저 정한다. #422 실측: 초안 +559B 를 두 번 압축해 +248B 로 넣었고, 보낸 것은 **git 이 소유한 이력 서술 둘**이지 판정 방법이 아니었다.
