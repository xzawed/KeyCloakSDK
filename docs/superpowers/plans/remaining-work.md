...
| 작업 패키지 | **170** (원장 유래 104 · 원장 밖 46 · 재스캔 신규 2 · 문서감사 신규 17 + 계수차 1) — 열림 **134** · 닫힘 **36** (2026-09-07 재측정) |
| 심각도 | high 27 · medium 78 · low 48 |
| 작업량 | S 69 · M 72 · L 12 |

<!-- doc-guard: kind=count source=work-packages -->
⚠️ **이 표를 판정에 쓰지 말 것 — 세 줄이 서로 맞지 않는다.** 체크박스 전수는 `170`(2026-09-07 기준 열림 134 · 닫힘 36)인데 심각도·작업량 행의 합은 **150**이다. 어긋난 채로 커밋돼 있었고(2026-09-06 확인), 어느 쪽이 옳은지는 원장을 다시 세야 정해진다. **수를 알아야 하면 위 두 명령을 돌린다.** ⚠️ 그리고 **「열려 있다」가 「아직 참이다」는 아니다** — 2026-09-07 재판정에서 20건 중 11건이 변동했다(진입점 참조).
...
- [ ] `jwks-response-not-validated` **[M/M · 절반 닫힘 2026-09-07]** JWKS 응답을 검증 없이 신뢰한다 — **남은 것은 Rust 의 상태코드 미확인뿐**(`error_for_status`·`.status()` **0건**, 실측 2026-09-07). JVM 둘의 본문 크기 무제한은 #400 이 `DEFAULT_HTTP_SIZE_LIMIT` 를 되살려 닫았다 · `rust/src/jwks.rs:34`
