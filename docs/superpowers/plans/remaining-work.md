...
1. **`seven-selftests-have-no-negative-control` 닫을지 재판정.** 지목한 파일을 전부 단언 단위로 봤다(조각 1–7). 남은 `test-provenance-gate` 는 이미 합성 입력을 먹인다 — 한 번 재고 닫는 쪽이 기본값이다.
...
- [ ] `seven-selftests-have-no-negative-control` **[H/L · 계수 정정 2026-09-09 · 조각 셋째 2026-09-23]** **여섯** 자가테스트가 라이브 상태만 단언한다 — 검출기를 지워도 통과한다 · `scripts/test/test-deploy-md.sh:7`
  - ✅ **일곱째 조각 — `test-publication-claims` 상태 표.** java `v*`·좌표 `…:keycloak-sdk` 가 형제 값에 흡수돼 이 파일에선 SILENT(check-docs 가 대신 잡던 **빌린 비공허성**) → 셀 경계로 3/3 CAUGHT.
  - ✅ **여섯째 조각 — `test-release-prerelease` 배선 대조.** 파일 전체 포함 검사라 배선 줄 둘을 주석으로 막아도 SILENT(→ RC 가 Latest) → 주석 줄을 벗겨 2/2 CAUGHT. `gh release create` 추출기는 이미 주석을 건너뛴다.
