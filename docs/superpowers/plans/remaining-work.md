...
- [ ] `guard-detection-surface-hand-narrowed` **[H/M · 계수 정정 2026-09-07 · 악화]** 가드의 탐지 표면이 손으로 좁혀져 있어 새 자리·새 문법이 조용히 통과한다 · `scripts/test/test-security-defaults.sh:311`
  ...
  - ⏸ **같은 부류의 남은 자리 둘 — 하네스의 언어 손 목록.** `harness/verify.sh:5` 의 `LANGS=("${@:-go dotnet node python java php rust ruby kotlin}")` 와 `harness/install/install-verify.sh:37` 의 `DEFAULT_LANGS` 가 아홉을 손으로 적는다. 열 번째 언어가 들어오면 **야간 하네스가 그 언어를 조용히 건너뛴다**. ⚠️ 여기서는 파생 원천이 다르다 — SDK universe 가 아니라 `harness/apps/*/` 다(하네스 앱이 없는 언어는 애초에 검증할 수 없다). 별도 변경으로 낸다(야간 경로라 required 밖이고 위험이 다르다).
