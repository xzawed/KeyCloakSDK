...
- [ ] **12 · `[!]` 미게시 분기 `claim_at`이 문서 결정과 모순** — `test-publication-claims.sh:318-328`이 `DEPLOY.md`·`roadmap`에 게시/미게시 **개수 문장**을 요구하는데, `DEPLOY.md:3`은 그 사실을 말하지 않기로 결정했다. 아홉 전부 게시라 지금은 도달 불가(초록). **10번째 언어를 추가하기 전에** 그 분기를 지우거나 조준을 `deploy-facts.sh`로 옮겨야 한다 — 그러지 않으면 언어 추가 PR이 문서와 가드 중 하나를 반드시 어긴다. 가드 변경이라 단독 PR.
