...
- [ ] **9.1 · rules 영문화 — 파일럿** (PR #C1) — `security.md`(2.7KB, 교차언어 불변식 소유자)로 규약을 굳힌다. **끝**: `test-security-defaults.sh` 92건 통과(가드가 한글 문구에 의존하지 않음을 실증).
- [ ] **9.2 · rules 영문화 — 배치 1** (PR #C2) — python·go·node·java(17.9KB).
...
⚠️ **배치로 쪼개는 이유**: 59KB가 측정으로 얻은 게차 서술이라 한 번에 옮기면 뉘앙스가 상한다. 각 배치마다 해당 언어 가드를 돌려 한글 문구 의존이 없었는지 확인한다(`check-docs.mjs`의 커버리지 게이트 파서는 `게이트|gate` 둘 다 받으므로 안전 — 실측).
