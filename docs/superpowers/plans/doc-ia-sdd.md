...
- [ ] **9.1b · 영문화로 드러난 가드 사각지대 복구** (PR #C2a) — `check-docs.mjs` 검사 6(`N개 언어` ↔ `DEPLOY_LANGS`)이 **한글 표기만** 본다. 영문 문서에 이미 언어 수 주장이 있고(실측 50건 중 총계 주장 37건) 그 전부가 무가드였다 — `9 languages`처럼 숫자형도 있어 10번째 언어가 들어오면 조용히 썩는다. **끝**: `all|across N languages`(숫자·수사 양쪽)를 대조. 총계 마커를 매치 안에 넣는 설계인 이유는 영어가 총계와 부분집합을 어순으로 구분하지 못하기 때문이다(마커 없이 확장하면 **오탐 8건** — 실측).
- [ ] **9.2 · rules 영문화 — 배치 1** (PR #C2b) — python·go·node·java(17.9KB). 9.1b 이후.
- [ ] **9.3 · rules 영문화 — 배치 2** (PR #C3) — ci·php·ruby(17.2KB).
- [ ] **9.4 · rules 영문화 — 배치 3** (PR #C4) — dotnet·kotlin·rust(21.5KB).
- [ ] **9.5 · WBS 3 재개 — 빌드 명령 3중복 제거** (PR #C5) — 9.2~9.4 완료 후. 영문화가 끝나야 영문 문서에서 rules로 보낼 수 있다.
