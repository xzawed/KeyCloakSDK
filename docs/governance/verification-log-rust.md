...
- **G2**: ✅ 단위 **34** GREEN(`cargo test`) + 통합 **1**(`full_flow`, `#[ignore]` — 실제 Keycloak 26.6, testcontainers) = **총 35**. (최종리뷰 fix wave `79ecf76`에서 `config.rs::whitespace_only_fields_err`·`token_provider.rs::fetches_with_custom_scopes` 2건 추가 + 기존 `oidc.rs::assembly`에 authorization URL 단언 추가 — oidc 테스트 개수 자체는 불변.)
- **G3**: ✅ `cargo llvm-cov --ignore-filename-regex '(auth|admin|client)\.rs' --fail-under-lines 90` — **실측 로직 모듈 라인 커버리지 94.85%**(855줄 중 44줄 미실행, 게이트 ≥90% 통과, exit 0). 네트워크 경계(`auth.rs`/`admin.rs`/`client.rs`)는 통합테스트로 검증하고 커버리지 게이트에서 제외(다른 6개 언어와 동일한 정책). 파일별: `error.rs` 100.00% · `oidc.rs` 100.00% · `jwks.rs` 96.05% · `token_provider.rs` 95.86% · `jwt.rs` 94.26% · `config.rs` 94.29% · `tokens.rs` 87.93%(함수 커버리지가 라인보다 낮은 이유 — `Debug` 마스킹의 `finish_non_exhaustive()` 분기·`is_expired` 일부 경계값 조합 미실행).
...
## 커버리지 실측 (Task 12 → 최종리뷰 fix wave `79ecf76` 반영)

`cargo-llvm-cov`(로컬 설치: `cargo install cargo-llvm-cov --locked`, `rustup component add llvm-tools-preview` 선행 필요)로 `cargo llvm-cov --ignore-filename-regex '(auth|admin|client)\.rs' --fail-under-lines 90`을 실행한 실측 결과(exit 0 — 게이트 통과). 아래 표는 최종리뷰 fix wave(`79ecf76` — admin 캐싱 provider 재배선 + `config.scopes` threading + 테스트 2건 추가) 이후 재측정한 수치다(Task 12 시점 원본 수치는 이 절 하단의 변동 요약 참고):

```
Filename            Regions  Missed Regions   Cover   Functions  Missed Functions  Executed   Lines  Missed Lines   Cover
config.rs               108               5  95.37%           8                 1  87.50%        70             4  94.29%
...
oidc.rs                  53               0 100.00%           8                 0 100.00%        38             0 100.00%
token_provider.rs       212              14  93.40%          19                 4  78.95%       145             6  95.86%
tokens.rs                77              11  85.71%           6                 1  83.33%        58             7  87.93%
--------------------------------------------------------------------------------------------------------------------
TOTAL                  1342              79  94.11%         110                 8  92.73%       855            44  94.85%
```

- **결과: 로직 모듈 라인 커버리지 94.85%**(855줄 중 44줄 미실행) — 게이트 ≥90% **통과**(exit code 0).
- 네트워크 경계 3개 파일(`auth.rs`/`admin.rs`/`client.rs`)은 `--ignore-filename-regex '(auth|admin|client)\.rs'`로 제외(다른 6개 언어와 동일하게 통합테스트로 검증) — 위 표에 이 3개 파일이 나타나지 않음이 제외가 정확히 적용됐음을 확인해준다. 나머지 로직 모듈(`config.rs`/`error.rs`/`jwks.rs`/`jwt.rs`/`oidc.rs`/`token_provider.rs`/`tokens.rs`)의 34개 단위테스트 전체(`jwt.rs`의 15개 포함)가 실측 대상이다.
- **Task 12 대비 변동**(fix wave `79ecf76`로 인한 이동만): `config.rs` 65→70 실행줄(신규 `whitespace_only_fields_err`, 93.85%→94.29%) · `oidc.rs` 36→38 실행줄(기존 `assembly`에 authorization URL 단언 추가, 100.00% 유지) · `token_provider.rs` 124→145 실행줄·5→6 미실행(신규 `fetches_with_custom_scopes` + `config.scopes` threading 분기 일부 미실행, 95.97%→95.86%로 미세 하락) · `error.rs`/`jwks.rs`/`jwt.rs`/`tokens.rs`는 불변. TOTAL 827→855줄(43→44 미실행), 94.80%→94.85%.
...
## 최종 전체브랜치 리뷰 (opus, 2026-07-06)

- **최종 리뷰**(opus 어드버서리얼 검증, 전체 브랜치 diff 대상): 판정 **0 Critical**. Important 1건 + Minor 2건 확정 → 단일 fix wave로 조치.
- **확정 결함 → 1 fix wave**(`79ecf76`):
  - **(Important) admin이 무캐시 `AuthClient`를 토큰 소스로 주입받고 있었다.** `KeycloakClient::new`가 `admin`에 `auth.clone() as Arc<dyn TokenProvider>`(즉 `AuthClient` 자신)를 넘기면, `AuthClient`는 `TokenProvider`를 구현하되 **호출마다 새 client-credentials 토큰을 발급**한다(캐시/single-flight 없음) — Go/Java/Python/Node/C#/PHP 6개 자매 SDK가 모두 지키는 §4 "캐시 가능한 client-credentials 토큰 재사용" 불변식을 Rust만 위반하고 있었다. 수정: admin 전용 `ClientCredentialsTokenProvider::new(config.clone(), http.clone())`를 별도로 생성해 주입 — 공유 `reqwest::Client`(커넥션 풀)는 그대로 재사용하되 토큰 자체는 캐싱+single-flight 경로를 탄다.
  - **(Minor) `config.scopes`가 하드코딩된 `"openid"`에 가려 무시되고 있었다.** `token_provider.rs::fetch`의 client-credentials 요청과 `auth.rs`의 authorization URL 빌더 둘 다 `"openid"` 리터럴을 썼다 — 사용자가 `KeycloakConfig`에 커스텀 스코프를 지정해도 실제 요청엔 반영되지 않는 no-op이었다. 수정: 두 지점 모두 `config.scopes`가 비어 있으면 `"openid"` 폴백, 아니면 실제 스코프 목록을 threading(`token_provider.rs`는 공백조인 문자열, `auth.rs`는 `Vec<Scope>`).
  - **(Minor) 회귀 커버리지 공백** — `config.rs`에 공백-only(whitespace-only) 필드 거부 테스트가 없었고(`.trim().is_empty()` 검증 자체는 존재했으나 미실증), `oidc.rs::assembly`가 `authorization()` 엔드포인트를 단언하지 않았다(token/introspection만 검증). `whitespace_only_fields_err`(신규) + 기존 `assembly`에 authorization 단언 추가로 보강.
- **결과**: 단위 32→34, 총 33→35, 커버리지 94.80%→94.85%(exit 0 유지). `cargo clippy --all-targets -- -D warnings`·`cargo fmt --all --check` 재검증 통과.
- **NON-ISSUE 확정**(리뷰에서 식별했으나 이번 fix wave 스코프 밖으로 명시 이연 — follow-up 후보):
  - `error.rs`의 `oauth_error`가 OAuth 에러 코드를 원문 그대로 노출(예: `invalid_client`) — 다른 5개 언어의 `Auth{oauth_error}` 패턴과 동형이라 그 자체는 §4 위반 아니나, 향후 분류 세분화(예: `AuthError::InvalidClient` 등 typed variant)를 검토할 여지.
  - `JwksStore`의 콜드스타트 경로가 최초 미해결 kid 조회 시 이중 fetch(중복 네트워크 호출) 가능성 — DoS-safe 불변식(rate-limit) 자체는 훼손되지 않으나 낭비적.
  - admin이 `KeycloakClient::new` 시점에 즉시(eager) 토큰을 발급/검증하지 않고 첫 admin 호출까지 지연 — 다른 언어(Java/PHP 등) 일부는 admin도 즉시 인증하므로 동형성 재검토 후보.
  - `AuthorizationRequest`에 `nonce` 필드가 없어(설계 의도 — id_token을 openidconnect로 검증하지 않으므로 nonce 소비 경로가 없음) 호출자가 nonce를 직접 다룰 수 없음 — 현재 설계상 의도된 것이나 문서화 강화 후보.
  - 이 4건은 Critical/Important가 아니므로 이번 fix wave에 포함하지 않았고, 별도 후속 이슈로 이연한다.
- **브랜치 상태**: `feature/rust-sdk` → `main` PR은 아직 미실행(human-gated). CI(로컬 재현 기준) GREEN.
