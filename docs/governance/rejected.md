...
<!-- 13094 → 13096 (2026-09-06, +2B). 규약 (1) 의 가장 순한 형태 — 늘어난 두 바이트는
     ...
     붙인 뒤 원격 사본 둘이 갱신되지 않았다). 산문 추가가 아니라 표기 변경이다. -->
<!-- 12548 → 13094 (2026-09-03, +546B = 새 기각 **1건**: php-semver V154 면제). 규약대로 행과 함께
     올렸고, 여유는 0 으로 둔다 — 다음 행도 반드시 이 숫자를 건드리게 하기 위해서다. 이 기각은
     php API 게이트에 정밀도를 넣으면서 **일부러 만들지 않은** 면제를 기록한 것이다: V154 오탐은
     실재하나(최소 A/B 로 재현) 도구가 PATCH 로 내어 MAJOR 게이트에 닿지 않으므로, 넣으면 시험되지
     않는 코드가 된다. 되살릴 조건을 명령으로 적어 다음 세션이 재판정할 수 있게 했다. -->
...
<!-- 신설 (2026-08-29): `process.md` §3 에서 **표만** 떼어 왔다. 체크리스트는 ③ 검토마다 쓰이는
...
기획·검토에서 **기각은 정상 출구**다. 판정 기준(체크리스트 `8`)과 「기각에는 되살릴 조건을 함께 적는다」는
...
<!-- GitHub Release 대칭화 행의 「23태그 중 15개」재판정 (2026-08-28 · 2026-09-22 재측정):
     `git ls-remote --tags origin | grep -v '\^{}' | grep -vc archive/` → 37 ·
     `gh release list --limit 60 | wc -l` → 13  ⇒ **24개 없음**.
     ⚠️ 종전 주석은 머리에 37/13 을 적고 괄호에는 28/10 을, 뺄셈에는 18 을 적어 **한 문장 안에서
     세 값이 서로 어긋나 있었다**(2026-09-22 감사). 지금은 세 자리가 같은 실행에서 나온 값이다.
     비율도 성격도 그대로라 **기각은 유지된다** — 전제가 뒤집힌 것이 아니므로 표의 수는 판정
     당시 스냅샷으로 남긴다. -->
