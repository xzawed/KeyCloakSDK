...
이 문서는 Keycloak 폴리글랏 SDK의 **언어 확장 전략과 우선순위**를 정의한다. 현재 지원 언어는 **Java**·**Python**·**TypeScript/Node**·**Go**·**C#/.NET**·**PHP**·**Rust**·**Ruby**·**Kotlin** 아홉 가지이며, 아홉 언어 모두 설계·구현·단위·통합·CI가 완료되고 실배포만 사람 게이트로 남아 있다. 이 로드맵은 (1) 언어 확장의 원칙, (2) 기존 SDK를 실제로 배포하기 위한 사전 단계(step-0), (3) 다음 확장 언어의 우선순위와 후보 클라이언트, (4) 언어별 현황 매트릭스를 정리한다.
...
| ✅ 완료 | **Rust** | `openidconnect`(ramosbugs) **=4.0.1** 수동 EndpointSet typestate + `jsonwebtoken` **=10.4.0** 자체 강화 | `keycloak` crate(kilork) **=26.6.2**(`reqwest12` feature) | **구현 완료(총 35 테스트, 현황 매트릭스 참조).** 확정 사실(딥리서치·구현 검증): `openidconnect` 4.0.1은 6개 엔드포인트 typestate 파라미터를 갖는 `CoreClient` 제네릭이라 auth/introspection/token만 `EndpointSet`으로 명시해 `?` 없이 exchange 빌더를 호출 가능한 구체 타입으로 좁힌다(id_token은 openidconnect 자체 검증기 대신 자체 `JwtValidator`가 access_token을 강화 검증하므로 JWKS는 비워 둔다). `keycloak` crate는 `reqwest12` feature로 reqwest 0.12 라인에 정렬해야 `openidconnect`(reqwest 0.12 고정)와 의존성이 맞는다 — 전역 reqwest 0.12 통일이 스캐폴딩 단계의 확정 사항. `jsonwebtoken`은 기본값이 안전하지 않아(`validate_nbf` 기본 false, `leeway` 기본 60초) 자체로 `validate_nbf=true`·`leeway=30`·`required_spec_claims=["exp","iss","aud"]`로 강화. DoS-safe JWKS(`JwksStore`)는 자체 구현(kid 캐시·미해결만 재조회·rate-limit을 재조회 *결정 시점*에 stamp — Go/Python 동형). `admin`↔`auth`는 `TokenProvider` trait(async, `#[async_trait]`)로만 접착, `keycloak::KeycloakError`는 경계(`map_admin`)에서 SDK `KeycloakError`로 변환. representation struct(`keycloak::types::*`) 노출은 Java/Go/C#/PHP와 동일한 문서화된 은닉성 예외. RUSTSEC-2023-0071(rsa Marvin, dev-dependency `rsa`는 테스트 키 생성 전용)은 개인키 복호화 사이드채널이라 우리 공개키 서명검증 전용 사용에는 무영향. |
| ✅ 완료 | **Ruby** | `rack-oauth2`(nov) **~>2.3** 그랜트+PKCE-passthrough + `jwt`(ruby-jwt) **~>3.2** 자체 강화 | (성숙한 gem 부재 — `faraday` **~>2.0**로 Admin REST 직접 래핑) | **구현 완료(총 74 테스트, 현황 매트릭스 참조).** 확정 사실(딥리서치·구현 검증): `rack-oauth2`(OIDF 인증 RP 저자 nov 유지)는 client-credentials/authorization-code+PKCE(passthrough — S256 검증자/챌린지는 SDK가 `SecureRandom`+`Digest::SHA256`+`Base64.urlsafe_encode64`로 손수 생성)/refresh 커버, introspection(RFC7662)·logout은 클라이언트측 미커버라 공유 `faraday`로 손수 POST. admin 후보였던 `looorent/keycloak-admin`(v1.1.7)은 외부 `TokenProvider` 주입 시임이 없어(자체 무캐시 토큰) §4 결합 규칙에 비호환 → 채택하지 않고 5리소스(users/clients/realms/roles/groups)를 `faraday`로 직접 구현(admin representation은 plain `Hash`로 통과 — Python과 동형). `jwt`(ruby-jwt)는 `algorithms`/`verify_iss`/`verify_aud`/`verify_expiration`/`verify_not_before`/`required_claims`/`leeway` 기본값이 전부 안전하지 않아 SDK가 전부 재정의하며, alg 핀은 키조회/서명검증 **이전**에 발동해(소스 확인) PHP류 헤더 사전-게이트가 불필요하다. DoS-safe JWKS(`JwksStore`)는 자체 구현(rate-limit gate를 재조회 *결정 시점*에 stamp — Go/Python/Rust 동형, nil 캐시에도 적용되도록 리뷰로 정정). 8번째(마지막) 언어로 E2E 신규 SDK 버그 0건(PHP·Rust에 이은 세 번째 무결함 사례). 통합테스트는 PHP와 동일하게 docker CLI 셸아웃(Windows native Ruby가 소켓 트랜스포트 미지원). |
| ✅ 완료 | **Kotlin** | `com.nimbusds:oauth2-oidc-sdk` **11.37.2**(Java SDK와 동일 JVM 라이브러리 재사용, PKCE S256) + `com.nimbusds:nimbus-jose-jwt` **10.9.1** 자체 강화 검증 | `org.keycloak:keycloak-admin-client` **26.0.10**(Java SDK와 동일) | **구현 완료(총 101 테스트, 현황 매트릭스 참조).** 확정 사실(딥리서치·구현 검증): 9번째 언어는 신규 클라이언트 라이브러리를 도입하지 않고 **JVM 자매 Java SDK가 이미 실 Keycloak으로 검증한 스택 그대로**(admin-client 26.0.10·oauth2-oidc-sdk 11.37.2·nimbus-jose-jwt 10.9.1)를 **코루틴 관용**으로 재래핑 — 신규 라이브러리 리스크 0. 모든 네트워크 메서드는 `suspend`, 블로킹 라이브러리 호출은 `runInterruptible(Dispatchers.IO)`(`onIo` 헬퍼)로 옮겨 취소 협조(cancellation)를 보장한다. `public fun interface TokenProvider { public suspend fun accessToken(): String }`가 Kotlin 2.2.20에서 컴파일됨을 실증(KT-40978 과거 SAM+suspend 제약 해소). `admin`은 `KeycloakBuilder` 내장 client-credentials 그랜트로 토큰을 자체 소유해 `auth`를 직접 알지 못한다(Java와 동일 결정 상속). JWT 검증(RS256 핀·`none` 거부·iss 정확일치·aud 포함·`exp` 필수·클록 스큐·DoS-safe JWKS `JWKSourceBuilder`)은 Task 6 opus 어드버서리얼 리뷰로 **SECURE 판정**(19 공격 프로브, 악용 가능 0). 빌드는 vanniktech `maven.publish` 0.37.0(Central Portal)+Kover 0.9.8(커버리지 게이트)+ktlint 14.2.0, 좌표 `io.github.xzawed:keycloak-sdk-kotlin`. |
...
| **Rust** | ✅ | ✅ (edition 2024 · async-only) | ✅ (34) | ✅ (1 E2E, Testcontainers) | ✅ | 🔒 사람 게이트 (총 35) |
| **Ruby** | ✅ | ✅ (sync-only · 예외 계급) | ✅ (73) | ✅ (1 E2E, docker CLI 셸아웃) | ✅ | 🔒 사람 게이트 (총 74) |
| **Kotlin** | ✅ | ✅ (coroutines `suspend` · JVM/Java 스택 재사용) | ✅ (100) | ✅ (1 E2E, Testcontainers) | ✅ | 🔒 사람 게이트 (총 101) |

**범례**: ✅ 완료 · 🔒 준비됨·사람 게이트(태그 push 대기) · 계획 미착수. Java 총 123개(단위 117 + 통합 6), Python 총 235개(단위 224 + 통합 11), Node 총 76개(단위 71 + 통합 5), Go 총 41개(단위 40 + 통합 1 E2E), C#/.NET 총 59개(단위 58 + 통합 1 E2E `Full_flow`), PHP 총 67개(단위 64 + 통합 3 `FullFlowIT`), Rust 총 35개(단위 34 + 통합 1 E2E `full_flow`), Ruby 총 74개(단위 73 + 통합 1 E2E `full_flow`), Kotlin 총 101개(단위 100 + 통합 1 E2E `FullFlowIT`)는 실측 기준이다. Java/Python/Node/Go/C#/Rust/Kotlin는 실제 Keycloak 26.6(.4)에 대한 Testcontainers 통합테스트를 포함하고, **PHP·Ruby는 docker CLI 셸아웃 폴백**(Windows native PHP/Ruby가 각각의 testcontainers 바인딩이 요구하는 `unix://` 소켓 트랜스포트를 지원하지 않아 `docker run`을 직접 구동 — CI의 ubuntu 러너에서는 동일하게 동작)으로 동일한 실제 Keycloak 26.6을 검증한다. 신규 언어는 착수 시 동일한 6개 열을 모두 채운 뒤에만 "완료"로 간주한다(depth-first). Node 배포는 `node-v*` 태그 → `.github/workflows/node-release.yml`(npm Trusted Publishing / OIDC + provenance, human-gated)로 트리거된다. **Go 배포는 레지스트리 없이 `go/v*` 태그 자체가 릴리스**(`proxy.golang.org` 자동 캐시) → `.github/workflows/go-release.yml`(검증 + GitHub Release + 프록시 워밍, human-gated). C#/.NET 배포는 `dotnet-v*` 태그 → `.github/workflows/dotnet-release.yml`(NuGet, `NUGET_API_KEY` 시크릿 필요, human-gated). **PHP 배포는 레지스트리 업로드가 아니라 Packagist가 GitHub 웹훅으로 태그를 감지해 자동 게시**(`php-v*` 태그 → `.github/workflows/php-release.yml`, 저장 시크릿 없음, Packagist에 `xzawed/keycloak-sdk` 저장소 등록은 1회 수동 선행, human-gated). Rust 배포는 `rust-v*` 태그 → `.github/workflows/rust-release.yml`(`cargo publish`, `CARGO_REGISTRY_TOKEN` 시크릿 필요, human-gated). Ruby 배포는 `ruby-v*` 태그 → `.github/workflows/ruby-release.yml`(RubyGems Trusted Publishing / OIDC, 저장 시크릿 없음 — 최초 1회는 API 키 수동 게시 또는 Trusted Publisher 사전등록 필요, human-gated). Kotlin 배포는 `kotlin-v*` 태그 → `.github/workflows/kotlin-release.yml`(Maven Central Central Portal, `publishToMavenCentral` 스테이징 후 사람이 Portal 콘솔에서 수동 release하는 2단계 게이트, in-memory GPG는 `ORG_GRADLE_PROJECT_*` 시크릿, human-gated).

이 매트릭스와 별개로, 8개 언어 SDK가 동일 HTTP 계약(v2)으로 실제 Keycloak에 대해 동형 동작하는지 검증·스코어링하는 하네스([`harness/`](../../harness/README.md), CI: [`.github/workflows/harness.yml`](../../.github/workflows/harness.yml))가 별도로 있다 — **8개 언어(Go·C#·Node·Python·Java·PHP·Rust·Ruby) 샘플 앱이 모두 완료**됐다(각 앱은 net/http·ASP.NET Core·Express 5·FastAPI·Spring Boot·Slim 4·axum·Rack/Puma 관용 프레임워크로 SDK를 소비한다). 레거시 `./run.sh`(k6 부하 실측·비교만 → `RESULTS.md`)에 더해 `./verify.sh go dotnet node python java php rust ruby`가 언어별 conformance(계약 준수)·security(JWT 하드닝 공격 프로브)·suites(각 SDK 단위테스트+커버리지+린트) 신호를 모아 **4차원 가중 스코어카드**(기능 30%·보안 30%·커버리지 20%·성능/동형성 20%, 등급 A~D)를 `report/SCORECARD.md`로 산출한다(⚠️ 성능·동형성 차원은 k6 실측 미연동 — conformance 통과율 근사, 후속). CI(harness.yml)는 안 A — PR/푸시엔 빠른 Go 스모크 게이트(`mvp-go`), 야간(schedule 03:00 UTC)·수동(workflow_dispatch)엔 5언어 k6 비교(`all-langs`)와 8언어 종합 검증·스코어링(`score-all`)을 실행한다.

또 하나, **설치·동작 검증 하네스**([`harness/install/`](../../harness/install/README.md), PR #21 병합)가 실배포 없이 **각 SDK를 게시 패키지처럼 Docker 로컬 레지스트리(Verdaccio·pypiserver·file-GOPROXY·BaGetter·nginx-.m2·gem-index·Satis·cargo-local-registry)에서 설치하고 실 Keycloak에 대해 동작(quickstart+conformance+security)까지 검증**한다 — 소스 경로 소비로는 검증되지 않는 배포 산출물의 설치 경로(매니페스트·메타데이터·의존성 해석)를 실배포 전 최종 확인. **8/8 언어 로컬 실측 전 셀 GREEN**(conformance 26/26·security 9/9). `cd harness/install && ./install-verify.sh`. CI `install-all` 잡(야간/수동·`INSTALL-MATRIX.md` 아티팩트).
