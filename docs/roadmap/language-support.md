...
이 매트릭스와 별개로, 9개 언어 SDK가 동일 HTTP 계약(v2)으로 실제 Keycloak에 대해 동형 동작하는지 검증·스코어링하는 하네스([`harness/`](../../harness/README.md), CI: [`.github/workflows/harness.yml`](../../.github/workflows/harness.yml))가 별도로 있다 — **9개 언어(Go·C#·Node·Python·Java·PHP·Rust·Ruby·Kotlin) 샘플 앱이 모두 완료**됐다(각 앱은 net/http·ASP.NET Core·Express 5·FastAPI·Spring Boot·Slim 4·axum·Rack/Puma·Ktor/Netty 관용 프레임워크로 SDK를 소비한다). 레거시 `./run.sh`(k6 부하 실측·비교만 → `RESULTS.md`)에 더해 `./verify.sh go dotnet node python java php rust ruby kotlin`가 언어별 conformance(계약 준수)·security(JWT 하드닝 공격 프로브)·suites(각 SDK 단위테스트+커버리지+린트) 신호를 모아 **4차원 가중 스코어카드**(기능 30%·보안 30%·커버리지 20%·성능/동형성 20%, 등급 A~D)를 `report/SCORECARD.md`로 산출한다(성능·동형성 차원은 **k6 연동됨** — validate p95 언어간 상대점수). CI(harness.yml)는 안 A — PR/푸시엔 빠른 Go 스모크 게이트(`mvp-go`), 야간(schedule 03:00 UTC)·수동(workflow_dispatch)엔 5언어 k6 비교(`all-langs`)와 9언어 종합 검증·스코어링(`score-all`)을 실행한다.

또 하나, **설치·동작 검증 하네스**([`harness/install/`](../../harness/install/README.md), PR #21 병합·Kotlin 편입 PR #26)가 실배포 없이 **각 SDK를 게시 패키지처럼 Docker 로컬 레지스트리(Verdaccio·pypiserver·file-GOPROXY·BaGetter·nginx-.m2·gem-index·Satis·cargo-local-registry·mvn-repo-kotlin)에서 설치하고 실 Keycloak에 대해 동작(quickstart+conformance+security)까지 검증**한다 — 소스 경로 소비로는 검증되지 않는 배포 산출물의 설치 경로(매니페스트·메타데이터·의존성 해석)를 실배포 전 최종 확인. **9/9 언어 로컬 실측 전 셀 GREEN**(conformance 26/26·security 9/9). `cd harness/install && ./install-verify.sh`. CI `install-all` 잡(야간/수동·`INSTALL-MATRIX.md` 아티팩트).
