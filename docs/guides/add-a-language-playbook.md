...
- [ ] **Fix the runtime floor** — settle on the language baseline, build tool, package manager, static type checker, linter, and formatter. (Reference: Java = **JDK 17+** — artifacts are compiled with `--release 17` and fail with `UnsupportedClassVersionError` on lower JDKs. Python = **3.10+**.) ⚠️ **The floor is the bytecode you emit, not the JDK you build with.** The JVM lanes build on 21 and emit 17; a floor that is only a build-tool setting drifts silently, so `scripts/check-jvm-bytecode-floor.mjs` re-reads the produced class files.
