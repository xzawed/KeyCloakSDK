...
# 소비자 하한**이다(java 17 · dotnet net8.0 · go 1.26 · node 22 · php 8.3 · python 3.10 ·
...
# ---- 규칙 10: 계수기를 **손으로 되감지** 않는다 ----
# 이 저장소의 음성/양성 대조군은 단언을 「검출기가 우는가」를 재는 도구로 쓰고, 그 프로브가
# 보고에 섞이지 않게 `_A_FAIL`/`_A_PASS` 를 되감는다. 그 되감기를 손으로 쓰면 `assert.sh` 의
# **파일 눈금은 안 되감겨** 서브셸 오라클이 거짓 양성을 낸다 — 그리고 그 오라클을 끄면 진짜
# 서브셸 증발이 다시 보이지 않게 된다.
#
# ⚠️ 실측(2026-09-23): 손 되감기 여덟 자리 때문에 `test-security-defaults.sh` 21 건 ·
# `test-publication-claims.sh` 6 건이 오라클에 걸렸다. 전부 `_a_save`/`_a_restore` 로 옮겼고,
# 계수(310 · 234)는 그대로였다. 이 규칙은 그 자리가 다시 생기는 것을 막는다.
#
# ⚠️ **직접 증가(`_A_FAIL=$((_A_FAIL+1))`)는 금지하지 않는다** — 그것은 부모 셸에서 실제로
# 일어난 판정이라 계수가 정직하다(눈금보다 커질 뿐이고, 오라클은 그 방향을 보지 않는다).
# 금지하는 것은 **저장된 값으로 되돌리는 대입** 하나다.
_ctr_hits=0
_ctr_bad=''
for f in "$DIR"/test-*.sh; do
  [ -f "$f" ] || continue
  _ctr_hits=$((_ctr_hits + 1))
  if grep -qE '_A_(FAIL|PASS)="\$[A-Za-z_]' "$f"; then _ctr_bad="$_ctr_bad $(basename "$f")"; fi
done
# 공허 방지 — 글롭이 깨지면 0 개를 훑고 통과한다. 실측 42(2026-09-23).
assert_eq "ok" "$(ok_if "$([ "$_ctr_hits" -ge 10 ] && echo 0 || echo 1)" "$_ctr_hits")" \
  "[규칙10] test-*.sh 를 ${_ctr_hits}개만 찾았다 — 글롭이 깨졌나? (0 개를 훑고 통과시키지 않는다)"
assert_eq "" "$(printf '%s' "$_ctr_bad" | sed 's/^ //')" \
  "[규칙10] 계수기를 손으로 되감는 자리가 있다 — \`_a_save\`/\`_a_restore\` 를 쓸 것(손 되감기는 파일 눈금을 남겨 서브셸 오라클을 거짓 양성으로 만든다)"
