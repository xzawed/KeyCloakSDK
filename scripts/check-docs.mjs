...
// 그래서 파일 존재만 확인되고 조각은 한 번도 검증된 적이 없다(`archive/docs-history-2026-08c`
// 태그의 `doc-ia-sdd.md` WBS 7이 그 사실을 기록해 두었으나 과제로 삼지는 않았다). CLAUDE.md는 그 위험을 명시한다 —
...
//   · **300B 이하**면 그 기록만으로 통과하고, 넘으면 **여기서 막힌다**.
// ⚠️ **「넘으면 사람 판정」이 오래 거짓이었다**(2026-09-11 정정). 이 검사에는 기록된 판정을
// 받아 주는 분기가 **없고**, 에러 문구는 「판정을 앵커 주석에 적고 리뷰에 올려라」라 인쇄해
// 존재하지 않는 통과 경로를 약속했다. 실측: 8b 가 선 뒤 착지한 인상은 **전부 ≤300B**
// (최대 269B)라 그 분기는 **한 번도 실행된 적이 없다** — 아무도 약속이 비었음을 몰랐다.
// ⚠️ 주석으로 여는 것은 **일부러 하지 않는다.** 주석은 작성자가 그냥 쓰면 되므로 그것으로
// 수치 게이트를 열면 저장소 최초의 **자기 승인** 구멍이 된다. 비교: `published=` 는 기록을
// 받아 주지만 **태그라는 외부 오라클과 다시 대조**한다(자유 서술이 아니다).
//   · 실제 통과 경로는 둘뿐이고 **둘 다 diff 에 보인다**:
//     (1) git 이 소유한 **경위 산문**을 커밋 메시지·PR 로 보낸다 — 실측 2회 모두 이 길로
//         해결됐고 **판정 방법은 한 글자도 깎이지 않았다**(#423 +559B→+248B · KGP +654B→+295B).
//     (2) 판정 방법 자체가 300B 를 넘으면 **이 상수를 같은 PR 에서 올린다** — 그 diff 가 곧
//         사람 판정이고, 주석과 달리 리뷰에서 보인다. ⚠️ 깎아서 맞추지 말 것: 압축이
//         「다시 재는 명령」을 지우면 그건 교환이 아니라 손실이다(CLAUDE.md).
    ...
    if (delta > BUDGET_RAISE_CAP) {
      errors.push(
        `${rel}: doc-budget 인상 +${delta}B 가 무심사 상한 ${BUDGET_RAISE_CAP}B 를 넘는다 — ` +
          `⚠️ 앵커 주석에 판정을 적는 것으로는 지나갈 수 없다(그런 분기가 없다). 길은 둘이다 — ` +
          `(1) git 이 소유한 경위 산문을 커밋 메시지·PR 로 보낸다(**판정 방법은 깎지 말 것**, 실측 2회 모두 이 길로 300B 안에 들어왔다) · ` +
          `(2) 판정 방법 자체가 안 들어가면 check-docs.mjs 의 BUDGET_RAISE_CAP 을 같은 PR 에서 올려라 — 그 diff 가 사람 판정이고 리뷰에서 보인다`,
      )
    }
  }
