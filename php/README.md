...
> **`1.2.0` is on Packagist** — `composer require xzawed/keycloak-sdk` resolves `1.2.0` under Composer's default `minimum-stability: stable`. A minor release of security fixes (present in `1.1.0`): an error raised on a malformed or hostile IdP response no longer prints tokens or the response body through `(string)$e`, `var_dump` or `print_r`. The `error_description` no longer reaches an SDK message, the message and `oauthError` carry the OAuth `error` only when it is shaped like an error code (`[a-z_]`, else `null`), and a non-string `access_token` is a `KeycloakAuthError` instead of a `\TypeError` holding the token response. **Added**: `Xzawed\Keycloak\Exception\SanitizedCause`. ⚠️ **On the auth, token-provider, JWKS and JWT paths, `getPrevious()` now returns a `SanitizedCause`, not the original Guzzle, league or firebase exception** (admin REST errors are unchanged) — it keeps the original class name (`originalClass`), code, file, line and a trace without arguments, and a message only when an audited library wrote it — so an `instanceof` check against those lower types no longer matches. A non-PSR-18 exception from the token provider or the JWKS fetch is now a `KeycloakTransportError`.
>
...
Two scope limits worth knowing. The JWKS cache and its rate limit are per-`JwksStore` in-memory state, so their reach follows your deployment model: under a long-running worker (Swoole, RoadRunner) they span requests, but under classic PHP-FPM every request builds a fresh store and the limit only binds within that one request. And masking covers this SDK's own `__toString()` and `json_encode()`, plus the engine dumps that honour `__debugInfo()` — `var_dump()`, `print_r()` and `debug_zval_dump()`. It does **not** cover `var_export()`, which reads the properties directly and prints the raw value. (⚠️ This line used to say `var_dump()` and `print_r()` printed the raw value too; `__debugInfo()` is implemented on the masked types and [SECURITY.md](https://github.com/xzawed/KeyCloakSDK/blob/main/SECURITY.md) already said so.) PHP also has no erasable string type, so the client secret lives in an ordinary `string` for its lifetime: masking is defence in depth, not a guarantee about your logs.
